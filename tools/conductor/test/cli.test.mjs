...
function setup(plans, lanes, { gate, maxOpenWorktrees = 3, spec = {}, stopRequested, idlePollMs } = {}) {
  ...
  writeFileSync(join(toolDir, "local.json"), JSON.stringify({ budget_usd: { implement: 5, fix: 3, review: 4, close: 3, merge: 2, repair: 3 }, run_budget_usd: 60, max_open_worktrees: maxOpenWorktrees }));
...
test("check refuses a local.json with no run_budget_usd", async () => {
  const { p, cli } = setup([{ number: "0101", phases: [dev("1")] }], { a: ["0101"] });
  writeFileSync(p.local, JSON.stringify({ budget_usd: { implement: 5, fix: 3, review: 4, close: 3, merge: 2, repair: 3 }, max_open_worktrees: 3 }));
