...
| `decay` `gamma` `wrap` `darken_center` `brighten` `darken` `solarize` `invert`, and **the video echo** `echo_zoom` / `echo_alpha` / `echo_orient` (Plan 0109) — a bundle emitted before that plan carries no register for them and must be re-converted | |
...
| **the figure contract, over every waveform the scene draws** ([ADR-0223](adrs/0223-the-figure-contract-reaches-a-custom-wave-because-the-source-applies-it-there.md)) — a custom wave passes through the source's `SmoothWave` unless it draws dots, which is the source's own exception, and its `value1`/`value2` carry the host's sample factor like every built-in figure's term | |
