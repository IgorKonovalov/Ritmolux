...
Что происходит между тем, как звук выходит из колонок, и тем, как на экране сдвигается фигура.
Написано для тех, кто хочет разобраться в устройстве, а не менять его: ничего отсюда не нужно знать,
чтобы пользоваться приложением или писать пресет.

## Два фронтенда, один мозг

Ritmolux — это один движок отрисовки, питать который можно двумя способами. Отдельное приложение
перехватывает то, что играет на машине; компоненту для foobar2000 сэмплы передаёт проигрыватель,
внутри которого он работает. Ни то ни другое до движка не доходит: он принимает поток PCM-кадров и
не знает, откуда они взялись. Эта единственная абстракция и позволяет одному и тому же визуальному
коду обслуживать оба фронтенда.
...
Если обратный вызов захвата на чём-то задерживается — на блокировке, выделении памяти, записи в
журнал, — в звуке слышен щелчок. Отсюда и правило: он не делает ничего из этого и отдаёт сэмплы за
...
движок; дальше по потоку их уже считают верными — горячий путь, который занимается перепроверкой,
занят не тем.
...
ниже примерно 246 Гц, потому что бин шириной 23,4 Гц не отличит бочку от басовой ноты. Эта цена
признана и ничем не компенсируется: длинное окно задерживает нижние полосы примерно на 85 мс. Так
устроено окно такой длины, и с этим мирятся, а не борются — путь от доли до реакции его не касается вовсе, так что
...
даёт всплеск на атаках, а не на громкости, поэтому тянущаяся нота его не запускает, а приглушённый
удар — запускает.
...
Всё это приходит в пресет набором из примерно двух десятков переменных только для чтения
...
функции, `select()` и то, что сообщается о неверном выражении, описаны в
[языке выражений](presets.md), а каждый параметр каждой системы есть в
...
никогда. Каждая сцена проходит через общую составную цепочку:
...
приходящим, и завершающее тональное отображение. Новой системе отрисовки всё это достаётся без дополнительной работы —
...
Кадр попадает на дисплей через графическую абстракцию — Metal на macOS, DX12 или Vulkan на Windows.
...
же огибающую атак и ту же оценку доли, в каждом прогоне и на любой машине: внутри него нет ни чтения
системных часов, ни случайности с незаданным зерном. Именно поэтому безоконная
[оснастка захвата](capturing.md) вообще может что-то утверждать о картинке.

Визуальная случайность никуда не делась — разлёт частиц, дрожание, — но её зерно задано явно,
поэтому сцена воспроизводится в точности. Что *не* детерминировано, так это темп кадров: сколько
кадров ляжет между двумя шагами анализа, зависит от вашего дисплея и вашей видеокарты, а пресет,
который понадеялся на обратное, выглядел бы иначе на мониторе 144 Гц. Поэтому `time` — это часы сцены в секундах, а не
счётчик кадров.
