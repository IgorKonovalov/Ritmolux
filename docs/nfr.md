...
- **`frame_ms_p99` spikes on a GPU resource rebuild, and a governor reading it bare would demote
  ...
  **Every figure above is an integrated-GPU figure, and here is the discrete one beside it**
  (Plan 0147 Phase 6, 2026-09-06). The table above is **not edited** — a corrected number and a
  ...
  **What moves is the spread, not just the level.** The discrete part holds the 165 Hz vsync cap
  ...
  (ADR-0071: a frame-time figure names the machine that produced it), so the floor commitment is met
