...
| 6 — The gate in TouchDesigner | human | done — four of five answered; the size sweep deferred | |
| 7 — Packaging and docs | dev | done | committed with this row |

### Notes

**Phase 6 — the gate, run by the user at the receiver.** Four of the five items answered; the
fifth is deferred with its reason.

| # | item | verdict |
|---|---|---|
| 1 | Colour fidelity | **ok** — judged against a `shot --frame-at 300` PNG of the same preset at the same size, in a `Movie File In` TOP beside the receiving one |
| 2 | Largest size and rate steady for ≥ 30 min | **1280x720 at 60 fps: 108,000 frames in 1800.00 s wall against 1799.99 s scene.** The *largest* was not swept — the user had no time, and it is recorded as outstanding rather than answered |
| 3 | Latency by eye | **good** |
| 4 | Resident-set growth | **+2.0 MB across 108,000 frames**, 280 MB peak, flat from ~frame 43,200 |
| 5 | Usable for the work | **yes** |

- **Item 1 settles ADR-0125's stated hazard**, which that ADR calls the likeliest way this ships
  looking wrong and which nothing in this repository can test. The engine's readback path is now
  checked end to end, where Phase 3 had only checked CPU-generated bytes.
- **Item 2 is bounded by the receiver, not by us.** The TouchDesigner Non-Commercial key caps the
  receiving TOP at 1280x1280, so 1920x1080 — the size ADR-0125's 8.29 MB/frame and ~498 MB/s figures
  are stated at — **cannot be confirmed end to end on this install** whoever sweeps it.
- Per-stage cost across the 30 minutes: **3.67-7.82 ms** render+readback against **0.27-0.58 ms**
  Spout send. The sink is not the limit, by an order of magnitude, which is the reading ADR-0125's
  zero-copy exit was to be decided on.

- **A 17-minute termination was observed once and did not reproduce, and its cause is unrecoverable
  because the instrument discarded it.** An earlier run of the same command stopped at 61,200 of
  108,000 frames with no summary line. That run was piped through a `grep` whose pattern did not
  include `^--stream`, which is how this mode's own errors begin — **so if it printed a reason, the
  filter deleted it**, and the pipeline's exit code was `grep`'s rather than the program's. The
  re-run, capturing everything, completed all 108,000 frames with no error line of any kind. Two
  differences between the runs are known and neither is established as the cause: a second
  `lmv.exe` occupied the GPU from ~9 minutes into the failed run, and its per-frame cost climbed
  from 4.04 ms to 11.40 ms while the clean run stayed at 3.67-7.82 ms. **Recorded as unexplained.**

**Phase 7 — done.**

- **The release job stages the pinned SDK and builds `--features spout`**, so the shipped `lmv.exe`
  carries `--stream` with nothing for the user to compile. Verified locally through the same command
  the job runs: the release binary lists both adapter rosters, which only compiles with the feature.
- **The zip carries `spout-license.txt` and the job verifies it is there.** Spout is linked
  **statically** (`SpoutDX_static.lib`), so no DLL travels and the only thing that can discharge
  clause 2 of the Simplified BSD licence is the notice.
- **Measured, since ADR-0125 estimated it:** the feature costs **178,688 bytes (175 KB)** —
  9,993,728 B without against 10,172,416 B with. The estimate of "~hundreds of KB" holds. **The
  shipped binary is now 9.70 MB against NFR §4's ~10 MB soft cap**, which is close whether or not
  this feature is in the build and is raised here for the close rather than acted on.
- `docs/capturing.md` gains a live-source section carrying the `Syphon Spout In` naming trap, the
  sender-name increment, the two-roster `--gpu` explanation, the measured figures above, and what
  the mode does not do. `README.md` documents `--stream`, `--list-adapters`, `--gpu` and the four
  companion flags. `packaging/windows/READ-ME-FIRST.md` gains a tester section including the
  hybrid-GPU rescue in plain language. `docs/nfr.md` §3 states that the streamed picture sits
  **outside** the 60 ms budget, because what a viewer of the receiving composite sees is governed by
  the receiver's pipeline.
- **`.github/workflows/release.yml` is touched, so the push needs the `workflow` OAuth scope.**
