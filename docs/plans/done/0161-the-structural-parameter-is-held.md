...
| 6 — the documentation sweep | dev | done | f14b019 |
...
- **`presets/` touched:** `presets/README.md` only (+210 / -32) — its generated parameter block,
  regenerated by the harness, plus the hand-written `[hold]` section and the Structural/Modal
  paragraph. **No `.toml` moved**, so the embedded set is byte-identical and nothing was seeded.
- **Plan header `Closes:`** none
- **What shipped:** a **feature**. The preset surface gains a `[hold]` table; `ParamSpec` gains a
  `kind`; the exported schema gains a `kind` per parameter (additively, `SCHEMA_VERSION` still 1,
  body hash moved); `--report` gains a held-bindings block and a `reachability.holds` list. No
  shipped preset uses any of it yet, so the rendered library is unchanged.
- **Operator docs touched:** `docs/presets.md` (the `[hold]` section, the anatomy list, the
  frame-history paragraph), `presets/README.md`, `docs/capturing.md` (the `--report` reference).
  `docs/configuration.md` untouched — this adds no flag, no env var and no config key.
- **Backlog probes (`node scripts/check-backlog-claims.mjs`):** exit **0** — *110 stated reductions
  still hold across all 48 live entries (8 unprobeable)*. It broke once during Phase 6 and the
  cause was mine: see the Phase 6 notes.
- **Full suite:** `cargo nextest run --workspace`, exit **0** — **1691 passed, 6 skipped**, 995 s.
  Run at the tip, after every phase had landed. The nine GPU suites ADR-0156 defers were also run
  at Phase 1 and again at Phase 3, which is where a moved golden would have been a stop.
- **Outstanding `human` phases:** none — all six phases are `dev`-owned and all six landed.
- **Lane base:** the branch is cut from `b459fb4`, and `main` moved to `237bd7b` while this ran
  (backlog 0193-0196 and the 0165 review fixes). No file in this lane's diff is one those commits
  touched, but the merge is not this lane's to make.
