...
**Mode 4 verdict: no blockers, no majors, three minors, one nit.** Verified rather than trusted: the
matcher's fence and code-span stripping across all three classes, all seven Phase 2 repairs read by
number rather than pattern-matched (none silently re-pointed), `capture_advance`'s three tests run
green, and `capture_audio`'s signature and behaviour unchanged as the `warmup_hops = 0` case.
ADR-0071 compliance is correct throughout — the 136.3 s -> 100.2 s figure names its box, adapter,
command and preset count and lives in a docstring, not an assertion, and the superseded 86 s ->
167 s pair is kept labelled as pre-Plan-0084 rather than deleted.

**Phase 4's real acceptance criterion failed and was accepted by the user at the escalation, not
absorbed.** The plan's premise — that the warm-up renders reach a DSP state needing no pixels — was
half wrong: they were also the *scene* warm-up, which `reactivity.rs` said in as many words. 35 of
36 per-band vectors moved, all upward; the lone identical one is `spectrum/Halo`, the only preset in
the set with no accumulating state. Nothing regressed and the tightest headroom in the library
roughly doubled (0.0287 -> 0.0504 against the 0.020 floor). Read any reactivity figure recorded
before 2026-08-13 as a different measurement rather than as drift.

The three minors, none repaired inside the plan: Phase 1's fixture done-when left no committed
artifact (the `root` argument exists for a fixture tree nothing in the repo passes); `docs/capturing.md`
described the gate's old capture call and its old ~1.8x conversion price in the exact paragraph that
tells a future author how to copy this pattern — repaired in the close series; and the byte-identity
test asserts the property that was never at risk (the render pass cannot reach the analyzer, which
is structural at `capture_api.rs:321`) while the property that actually moved — GPU-integrated scene
state — is documented in three places but asserted nowhere. The nit: the use-class narrowing is
deliberately blind to the mirror failure where a definition block is deleted outright.
