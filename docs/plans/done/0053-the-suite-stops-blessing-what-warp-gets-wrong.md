...
> single-uniform layouts and rendered the fragment field a **flat grey** on WARP; `blend-bind-layout`
> collided with `trails-bind-layout` and rendered a dissolve between two `trails`-binding presets
> wrong. Both are fixed by an explicit `min_binding_size` — measured, isolated against a
> no-collision control, and load-bearing (reverting either re-collides its pair and fails the new
> assertion; verified at this review). Nine colliding pairs became four allowlisted ones. That
> **falsifies ADR-0058's own "nothing is observed to be wrong"** — see its `Outcome`.
>
> Verified at the close: `fmt`, `clippy --workspace --all-targets -D warnings`, and
> `nextest --workspace` **646/646 green**; `check-doc-links.mjs` clean. **Zero baselines moved** —
> two `LMV_BLESS` runs over `golden` + `composite` + `line_joints` + `attractor_trails` differing
> *only* by reverting the two production fixes came back hash-identical across all 20 PNGs, which is
> a tighter control than the clean-`main` one this plan inherited because it holds everything else
> fixed. `attractor_trails.png` reproduces byte-identically (mean 0.0000, outlier 0).
>
> **Phase 3 is tagged `human` and was run by `dev` under the user's explicit authorization at the
> gate.** The tag was correct; the delegation was consented and is recorded here rather than
> back-fitted into the plan.
>
> Two majors, both consequences of the plan's own success, left as findings rather than repaired
> here: **the allowlist's `RIG` names neither adapter** (this repo established five days earlier, in
> ADR-0074's `Outcome`, that *this box's WARP 10.0.19041 is the outlier* and CI's 10.0.26100 behaves
> like hardware — so evidence recorded as "DX12 hardware vs WARP" grants permission on a build it
> was never measured against; the identities cannot be reconstructed after the fact, which is why
> this is `dev`'s to fill); and **the fixed quirk is still asserted as live in two other places** —
> `core/tests/background_composite.rs`'s module docs and its software-adapter skip, which may now be
> liftable and would hand CI a check it has never run. `docs/capturing.md`'s row for that test now
> says so; the test's own docs do not.
> **Premise correction, 2026-08-04 (at Plan [0060](0060-a-test-number-states-a-property-or-names-its-machine.md)'s
