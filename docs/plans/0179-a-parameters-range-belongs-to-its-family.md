...
**Lane:** `C:\Users\Igor Konovalov\WORK\rlx-plan-0179` on branch
`plan-0179-a-parameters-range-belongs-to-its-family`.
...
- **Phase 1 also rewrote `presets/preset.schema.json`**, which the phase's file list does not name.
  It is the generic editor schema, generated by the same `RLX_UPDATE_PRESET_SCHEMA=1` switch the
  phase does name, and it carries the shared `param.deposit_arms` definition whose hover text states
  the kind.
- **Phase 2 also edited `core/tests/suite/preset.rs`**, which Phase 3's file list names and Phase
  2's does not. The attractor's `a`..`d` gained a family cell in the reference while the document
  still writes `"range":null`, and `the_published_reference_and_the_exported_schema_agree` asserted
  those two states were equivalent. Its range comparison now branches on whether the reference cell
  is a family cell; the `null` ⇔ blank equivalence is unchanged for every other parameter. Without
  it Phase 2's commit would have been red.
