...
| [0116](0116-the-sanity-lens-finds-the-ground.md) | The sanity lens finds the ground | in-progress | dev, human | **Must land before [0113](0113-the-engine-paints-a-canvas.md) Phase 6**, where the emptying canvas arrives; 0113 Phases 3-5 are unaffected and the lanes run in parallel until then. Carries [ADR-0126](../adrs/0126-the-sanity-lens-measures-departure-from-the-frames-own-ground.md) (proposed, **Outcome**), which re-bases `is_lit` on a ground **derived from the frame** rather than a hardcoded `BLACK`. **Phase 1 ran and Phase 2 chose `modal_luma`** — it re-bases 17 of 41 presets and changes **no verdict at either excitation**, which falsifies ADR-0126's reading of its own count. Two residues the ground structurally cannot reach — the flat-graphic false positive, and four groundless luminous fields still at `coverage` 1.0 — moved into **Phases 8-9** on 2026-08-26, carrying [ADR-0128](../adrs/0128-a-tonally-flat-picture-is-a-blot-only-if-it-is-also-structureless.md) (proposed): a picture is a blot only if it is flat **and** structureless. Phase 8 is a second mechanical stop gate. Closes [design-backlog 0128](../design-backlog.md). |
...
| [0113](0113-the-engine-paints-a-canvas.md) | The engine paints a canvas | approved | dev, human | **A twelfth system, `shape_collage` — the engine's first *graphic* world rather than a luminous one, from the user's Russian-avant-garde reference set.** Carries [ADR-0123](../adrs/0123-a-flat-graphic-scene-paints-its-own-paper-and-composites-opaque-elements-in-one-pass.md) (proposed), whose finding is that this needs **no composite change at all**: a fullscreen scene emitting alpha 1 already holds the backdrop out (measured, Plan 0091 Phase 1), the tonemap is the identity below `KNEE = 0.6` so flat colour survives byte-identical, and bloom's threshold sits above that knee so hard edges come free. **Phase 3 is a stop gate** — the draw path is O(elements) per pixel and the bounding-box reject saves the body, not the loop step, so Phase 2 measures before Phase 4 builds; it can route the plan to ADR-0123's Alternative A or B. **Phase 5 is a second human gate**, picking the layout grammar from rendered samples. Advances [design-backlog 0069](../design-backlog.md) without closing it: occlusion works *within* a scene, cross-scene ordering still does not exist. **Amended 2026-08-25: Phase 6b added, Phase 6 blocked on [0116](0116-the-sanity-lens-finds-the-ground.md)** — the emptying canvas it builds is measured by nothing today. Watch [ADR-0037](../adrs/0037-internal-grid-is-a-resolution-not-a-shape.md) — this computes screen-destined geometry, and Phase 1's 1280x800 check is the only thing that discriminates it. |
