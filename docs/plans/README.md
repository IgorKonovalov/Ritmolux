...
| [0108](0108-the-milkdrop-import-gets-its-tone-back.md) | The MilkDrop import gets its tone back | in-progress | human | **Every `dev` phase landed 2026-08-17 and was reviewed the same day (no blockers, two majors repaired at the review). What is left is the two `human` look-gates, and only those.** Phase 1 shipped ADR-0118's quantizer — the feedback field reaches exact zero and `warp_mesh.png` did not move. Phases 4 and 5 fixed the `wave_usedots` beads, `wave_mode 5`'s mixed trace, and *chasers 19 Portal*'s inert mirror (a wave's per-point state was being reset between points, reaching **3 368 of 6 347** custom-wave corpus presets). **Phase 3 reproduced the reflection seam and deliberately did not fix it** — the sign it implicates is carried in three places and settling it needs the reference on screen. **The plan stays live because Phase 2 is a routing cut point**: if the banding reads worse than the wash did, it returns to Phase 1 for ADR-0118's Alternative D. Phase 6 also owes answers on Phase 3's sign, Phase 4's `time * 0.05`, and whether a wave's per-point state survives the frame. Backlog 0106 and 0107 stay live, part discharged. Contends with nothing. |
| [0106](0106-the-frame-stream-passes-through-a-diffusion-model.md) | The frame stream passes through a diffusion model | draft | dev, human | **Phase 1 is a throwaway spike and Phase 2 is a stop condition** — if a diffused attractor boils, the plan ends there having cost an afternoon and nothing is built. **Fully unblocked: [0101](done/0101-the-engine-renders-a-music-video.md) closed 2026-08-17**, so the Y4M stream every later phase needs exists on stdout today and the `shot/` lane is free. **No ADR yet, deliberately** — it is written between Phases 2 and 3, against the spike's evidence rather than a guess. Ships a script, no weights and no runtime, so `lmv.exe` and the release zip do not change; `core/` is untouched. Contends with nothing (`tools/` + `docs/` only). |
...
- **Approved 2026-08-16 from a user request, and it collides with exactly one plan:
  [0107](0107-the-foobar-menu-picks-a-preset.md).** It rewrites the same `WM_CONTEXTMENU` handler
  [0103](0103-the-project-gets-an-audience.md)'s amended Phase 1 does, so **the two never run in
  parallel lanes** — either order is correct, and the plan's own Risks section states the trade.
  The tiebreak is availability rather than architecture: 0107 is takeable today (its ABI phase
  depends on nothing and its ADR is written), while 0103's Phase 1 is a surface-lifetime design
