...
**Next free number: 0133** (ADRs are a separate sequence — next free there is **0145**.)
...
| [0129](0129-the-build-stops-being-paid-three-times.md) | The build stops being paid three times | approved | dev | **Next up, after [0123] and [0127] close** (user, 2026-08-28). Not a feature — **build infrastructure, and it makes every plan behind it cheaper**. Three worktrees hold **23.2 GB of `target/`**: one 305-crate dependency graph compiled three times, optimized (`opt-level = 2` on deps), linked across 41 test binaries each. Carries [ADR-0141](../adrs/0141-one-artifact-store-serves-every-lane.md) (proposed): one shared artifact store plus `rust-lld`, configured in a **machine-local `.cargo/config.toml` above the worktrees** — never committed, because CI's `Swatinem/rust-cache` caches `./target`. **Zero coverage change by construction**; the two levers that would have cost coverage (GPU suites deferred to CI, the 41 test binaries consolidated) are explicitly out. Revokes exactly one ADR-0053 positive: lanes now serialize on cargo's lock. Phase 5 repairs `plugin-foobar/build.ps1:31`, the one script the redirect breaks. |
| [0130](0130-the-audio-input-becomes-an-operator-surface.md) | The audio input becomes an operator surface | approved | dev, human | Carries [ADR-0142](../adrs/0142-the-audio-input-is-switched-live-and-the-shell-owns-the-policy.md) (proposed): the capture stream becomes restartable at runtime, with **the shell owning every policy decision and the capture thread owning none** (it gains one `AtomicBool` store and nothing else — NFR section 5). `[input] mode`/`device` have worked since Plan 0009 Phase 2 and are reachable only by hand-editing TOML; this adds `--input`/`--device` and two live `S`-menu rows. **Supersedes a Plan 0083 premise** — `CaptureVerdict` stops being decided once at startup and becomes current state. **Phase 5 is a `human` on-device gate** and it is the only evidence the plan will get: the swap hitch, the unplug recovery, and whether `AUDCLNT_E_DEVICE_INVALIDATED` is even the code a real unplug produces — if it is instead a silent zero-frames stream, Phase 4 never fires and the honest fix is a separate decision. Windows-only in substance; macOS and Linux get read-only rows. Makes design-backlog 0032 (analysis windows sized in samples) reachable by keypress without fixing it. |
| [0131](0131-the-operator-gets-a-console.md) | The operator gets a console | draft | dev, human | Carries [ADR-0143](../adrs/0143-the-operator-console-is-a-second-surface-and-the-shell-owns-its-meaning.md) (proposed): a second winit window on the operator's display, its surface attached to the renderer's **existing** device — the frame is drawn once, copied exactly to the output swapchain and blitted scaled into the console, so no dissolve is double-advanced and nothing reads back to the CPU. **The Plan 0115 dependency named in the interview is withdrawn**: that plan's tap re-renders through `draw_frame` and returns pixels on the CPU, so it cannot serve a display-loop thumbnail — the ordering constraints are [0126](0126-the-large-files-split-along-their-seams.md) Phase 7 (`main.rs` becomes shell glue) and the live [0130](0130-the-audio-input-becomes-an-operator-surface.md) lane, which is rewriting `settings.rs` and `main.rs` now. Mirrors the two existing modals rather than rewriting them, and **adds no core primitive** — blackout, freeze and a cue monitor are all explicitly out. Phase 6 is a `human` gate and the only evidence the plan gets for what a second swapchain costs the output, and for the dual-GPU surface path CI has one adapter and cannot see. |
| [0132](0132-the-lighting-rig-follows-the-visuals.md) | The lighting rig follows the visuals | approved | dev, human | **Amended 2026-08-28 against a 2026-08-29 show date: the single Phase 1 stop gate is split into 1a (the rig half we do not own — NOT a gate, and runnable concurrently with Phase 2) and 1b (the NDI probe + licence read, which gates Phases 5-7 alone), so `dev` can start immediately.** Carries [ADR-0144](../adrs/0144-the-lighting-feed-is-a-resolved-ndi-sender-and-a-fixed-osc-telemetry-set.md) (proposed): two optional `standalone` sinks feeding **Resolume Arena on a separate machine over gigabit Ethernet**, which owns the fixture patch and emits the Art-Net. **The OSC half touches no `core` code** — the shell already holds the `AnalysisFrame` — so **Phases 1a-4 are the entire show-night deliverable** and end at a lamp moving on the beat. Phases 5-7 hard-depend on [0115](0115-the-engine-becomes-a-live-video-source.md) Phase 2's frame tap, which this plan extends but does not build, and are **not reachable by the show**; the NDI licence is the one finding that can kill the video half outright, and outcome (b) still ships a working feature. |
