...
| [0132](0132-the-lighting-rig-follows-the-visuals.md) | The lighting rig follows the visuals | approved | dev, human | Carries [ADR-0144](../adrs/0144-the-lighting-feed-is-a-resolved-ndi-sender-and-a-fixed-osc-telemetry-set.md) (proposed): two optional `standalone` sinks feeding **Resolume Arena on a separate machine over gigabit Ethernet**, which owns the fixture patch and emits the Art-Net. **The OSC half touches no `core` code** — the shell already holds the `AnalysisFrame` — so Phases 2-4 stand alone and end at a lamp moving on the beat. **Phase 1 is a `human` stop gate before `dev` starts**, settling six unverified facts with generic tools and separating three outcomes; the NDI licence is the one that can kill the video half, and outcome (b) still ships a working feature. Phases 5-7 hard-depend on [0115](0115-the-engine-becomes-a-live-video-source.md) Phase 2's frame tap, which this plan extends but does not build. |
