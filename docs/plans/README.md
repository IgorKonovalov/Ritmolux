...
|------|-------|--------|-------|-----------------|
| [0096](0096-the-hud-gets-out-of-the-way.md) | The HUD gets out of the way | approved | dev | **The smallest plan on the roster and the only one touching no core code** — three shell-local UX nits from a user report (the preset name is painted under the F3 panel and both modals; `Escape` reaches nothing outside a modal; the name cannot be turned off). Every file is under `standalone/`, so it contends with nothing here. Two subtleties are written into the phases: `Escape` must be handled *after* the modal branches or it steals the menu-close, and the settings row takes `SettingsRow::ALL` from 8 to 9, which stales the "eight rows fit any window" comment at `main.rs:783`. |
