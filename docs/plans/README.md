...
|------|-------|--------|-------|-----------------|
| [0110](0110-the-shader-surface-stops-being-invisible.md) | The shader surface stops being invisible | approved | dev, human | **CI's `coverage` job has been red since 2026-08-16 and is the only red one** — `warp_mesh/shader.rs` is 719 lines at 0.00 %, unreached because every fixture drives the mesh from EEL2 bytecode and none carries WGSL. The arithmetic sets the scope: **746 missed lines must go and the two files in scope hold 990**, so the margin is thin and the floor stays at 91 by decision rather than by drift. |
