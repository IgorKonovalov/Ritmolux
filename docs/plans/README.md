...
**Next free number: 0074** (ADRs are a separate sequence — next free there is **0088**.)
...
| [0065](0065-the-mandala-interior.md) | The mandala interior: `star_pattern` stops being hollow (a `rings` roster of motif / count / radius / scale / phase on `[generator]`) | **approved 2026-08-04** — ready for `dev`, gated by nothing; the fourth reference image; [ADR-0079](../adrs/0079-the-mandala-interior-is-rings-of-motifs-inside-star-pattern.md). **Closes the still-open half of [backlog 0034 → 0007](../design-backlog.md)** — the "hollow ring" finding with the user's standing *invest, do not cut* call on it, live since 2026-07-26 and never specified until the reference image arrived. Line geometry, so it **shares nothing** with [0055] or [0064] and can run in parallel with either. **Phase 3 is `human`** (pick the motif roster from a rendered grid) and gates Phases 4-6. Watch the `animation.rs` gate: a ring mandala is *more* rotationally symmetric than `star_rosette`, so spin alone will not pass it ([backlog 0009](../design-backlog.md)) — Phase 4 designs around it. Moves **no** golden baseline at all. **Phases 1-5 landed 2026-08-06** on `plan-0065-mandala-interior` (`33e5efc`/`1904469`/`3c0e56a`/`419418f`/`a35485a`); three mandala presets ship tuned by eye and **`sanity.rs` is red on all three** (0.2442/0.2505/0.2544 against a `star_pattern` floor of 0.34). **Architect disposition 2026-08-06: not a block.** The floor is *derived from the shipped library* at half each family's sparsest member, `MAX_FLOOR_SLACK` exists to force re-derivation when that minimum moves, and this plan moved it — so a **Phase 7** (`dev`) was added to re-derive it (expected `0.12`, slack 2.04) after Phase 6 and after the merge. [Backlog 0071](../design-backlog.md) stays open at medium-high as the measure's replacement, which re-derivation is *not*. **Still owed:** Phase 6 (`human`, unrun), then Phase 7, then the close. Two merge hazards: this lane and `main` each minted design-backlog **`0070`** on 2026-08-06 (lane's `0070`-`0072` renumber to `0071`-`0073`, and the archive conflicts), and `main` is at `v0.41.0` so the close bump is **`minor` → `v0.42.0`** | dev, human |
...
### The order, as of 2026-08-05

Everything below this list is the reasoning. **The one hard ordering constraint in this table has
been discharged** — [0055] had to precede [0064] and [0055] has closed — so what remains is
preference, and a session may take any plan here.
...
| 4 | [0065] | **Gated by nothing and shares no file with anything** — line geometry against a post chain and a particle scene. The safest parallel lane, and it can run *alongside* 1-3 rather than after them. **In flight** — Phases 1-5 landed 2026-08-06 in `WORK/lmv-plan-0065`; Phase 6 (`human`) and the added Phase 7 remain | No — Phase 3 is `human`, mid-plan |
...
| [0065] | 2 (Phases 1-2) | **mid-plan** — Phase 3 gates Phases 4-6. **Held exactly**: Phases 1-2, stop, then Phases 4-5 in one sitting after the verdict |
...
real audio, on both test scenes, across all five arms, at the user's own judgement — and it is the
only record of *why* three treatments ship and two do not:
...
**All of it shipped, and one predicted cost did not arrive.** Both adoptions landed
(`attractor_leviathan` → `tile` including the lifted `zoom` pin, `fragment_kaleido` → `squash`), and
the roster is `0` falloff / `1` tile / `2` squash with `tile` the default. But the wholesale
re-bless the fourth bullet accepted **did not materialise**, and the lane proved that rather than
assuming it: `composite_kaleido.png` is byte-identical under `tile` because it is a centred figure
over an **empty border**, and mirroring empty content outward yields empty content. That is now
recorded in the fixture header as a *property* — the baseline pins the fold's geometry and is
structurally blind to the edge treatment, which is why `composite_kaleido_squash.png` was added to
pin the radius map separately. **The eleven other fold-binding presets did change look**, by default
rather than by an author, and retuning them is [backlog 0058](../design-backlog.md) — the one thing
this verdict created that is still owed.
...
**A third premise fell in implementation, and it is the one worth remembering.** [0059] Phase 1
landed the correct x–z basis, and Lorenz still rendered as an X — because the *scene's own trail*
mirrors itself. The decay pass samples the accumulation target with the unflipped fullscreen
prelude while the draw pass writes that target in clip space, so the feedback re-reads its history
vertically mirrored and every attractor has been rendering as `figure ∪ mirror(figure)` for the life
of the scene. Both [0057] Phase 4 and [ADR-0068](../adrs/0068-the-projection-basis-is-a-per-family-property.md)
read the shipped X as "the two lobes edge-on", which is a correct reading of the *wrong* figure —
the doubling was doing at least as much of the work as the basis. It survived every gate because a
mirror-symmetric output conceals its own symptom, and it was caught by asking the ADR-0037 question
in a new place: `pan_y` returns **two mirror copies** where a translation can only return one.
[ADR-0070](../adrs/0070-a-feedback-pass-addresses-its-own-target-in-framebuffer-space.md) retires
the prelude and adopts that diagnostic as the gate.
...
**Four plans came out of the backlog on 2026-08-01 and 2026-08-02, and they are independent of
each other and of the render roadmap.** Take them in whatever order suits the session ([0055], the
fourth, is described under the fold heading below):
