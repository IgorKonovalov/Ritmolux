...
| [0112](0112-the-handoff-stops-being-a-chat-message.md) | The handoff stops being a chat message | approved | dev | **Harness only — no Rust, no presets, no scripts, no CI; moves no pixels.** `dev` writes its close brief into the plan as a `## Implementation log` — one row per phase, inside that phase's own commit — instead of pasting it into chat, and Mode 4 reads it as the brief while still verifying every claim against the tree. Carries [ADR-0120](../adrs/0120-the-close-brief-is-a-section-of-the-plan.md) (proposed). **The log is deliberately thin: observations, never conclusions** — no per-criterion pass list, no self-assessment — because a written brief anchors a reviewer harder than a chat message does; but a deviation or an unmet done-when is always disclosed. **Low risk: it specifies a practice `dev` already invented ad hoc in four closed plans** (0100, 0108, 0110, 0111). The payoff is that a finished worktree lane becomes self-describing and can wait its turn, which is what makes parallel lanes practical. All three phases edit `.claude/skills/**` and `CLAUDE.md`; the owner vocabulary still has no word for that, so all three are tagged `dev` on the Plan 0067 Phase 4 precedent. |
