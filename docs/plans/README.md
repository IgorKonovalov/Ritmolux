...
**Next free number: 0121** (ADRs are a separate sequence — next free there is **0132**.)
...
|------|-------|--------|-------|-----------------|
| [0120](0120-the-standalone-ships-on-ubuntu.md) | The standalone ships on Ubuntu | draft | dev, human | Carries [ADR-0131](../adrs/0131-the-linux-standalone-captures-through-pulseaudios-simple-api.md) (proposed): a third capture arm on PulseAudio's simple API, an `ubuntu-latest` CI arm, and a fourth release artifact. **Phase 1 is a `human` stop gate that runs before `dev`** — it tests on the real box whether `@DEFAULT_MONITOR@` resolves under `pipewire-pulse`, which the whole backend rests on, and a negative answer re-plans rather than redirects. Touches no `core/` code; the third `cfg` arm it compiles has never been built by anything. |
...
| [0087](0087-the-line-renderer-draws-a-curve.md) | The line renderer draws a curve | in-progress | dev, human | **Phases 1, 1b, 2, 3 and 4 landed and were reviewed; 5, 6 and 7 are unstarted and Phase 5 stays green-lit.** **Its code reached `main` on 2026-08-26, inside [0114](done/0114-the-line-stroke-reads-as-a-drawn-line.md)'s close merge** — 0114 was branched off this lane (its Phase 2 needs the arc fragment) and carried phases 1-4 in with it, so the arc primitive ships and the remaining phases resume from `main` rather than from this branch. **The park is now discharged**: 0114 closed, so the stroke the biarc chain is to be judged on is the shipped one. Both gates cleared: Phase 3's stop condition did **not** fire (a 40-member arc ring costs +0.036 ms sparse / +0.270 ms dense against a 16.67 ms budget, on a discrete laptop GPU — that reading can convict and cannot acquit the NFR §1 iGPU floor), and Phase 4's look gate returned *“circles looks fine but blurred”*, which green-lights **Phase 5, the biarc chain**. Two obligations this row carried turned out **not to be owed**: no baseline moved (no shipped preset declares a `rings` roster, so the re-bless never existed — and 28 was already stale, the count is 18 in `golden.rs` of 33 files), and the arc pipeline **shares** the segment pipelines' bind layout, so ADR-0058 has no new entry to make. Watch [ADR-0037](../adrs/0037-internal-grid-is-a-resolution-not-a-shape.md): the arc control runs at 4:3 and is verified to bite. |
