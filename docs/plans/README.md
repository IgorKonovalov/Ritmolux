...
**Next free number: 0098** (ADRs are a separate sequence — next free there is **0111**.)
...
|------|-------|--------|-------|-----------------|
| [0096](0096-the-hud-gets-out-of-the-way.md) | The HUD gets out of the way | draft | dev | **The smallest plan on the roster and the only one touching no core code** — three shell-local UX nits from a user report (the preset name is painted under the F3 panel and both modals; `Escape` reaches nothing outside a modal; the name cannot be turned off). Every file is under `standalone/`, so it contends with nothing here. Two subtleties are written into the phases: `Escape` must be handled *after* the modal branches or it steals the menu-close, and the settings row takes `SettingsRow::ALL` from 8 to 9, which stales the "eight rows fit any window" comment at `main.rs:783`. |
| [0097](0097-the-track-announces-itself.md) | The track announces itself | draft | dev, human | **Carries [ADR-0110](../adrs/0110-now-playing-is-a-shell-supplied-string-and-the-core-owns-the-banner.md) and is split so the expensive half can be abandoned.** Phases 1-3 ship a complete standalone feature with **no ABI change and no new crate** (SMTC is a feature flag on the `windows` dep the shell already has); Phase 4 widens the C ABI to a thirteenth function, bumps `LMV_ABI_VERSION`, and **measures the plugin DLL against NFR 4's ~10 MB cap with a stop condition** — if glyphon blows it, the plan stops there and Phases 1-3 stand. Shares `[hud]` in `config.toml` with [0096](0096-the-hud-gets-out-of-the-way.md); whichever lands first creates the section. |
