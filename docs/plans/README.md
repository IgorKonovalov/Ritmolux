...
1. **[0109](0109-the-milkdrop-import-gets-its-geometry-back.md)** — **in progress**; 0110 closed
   ...
   subsystem twice while the context is warm. **Amended 2026-08-19 with a Phase 6** carrying 0110's
   one major — `hygiene.rs` cannot see a `#[path]`-declared test module, so `shader_tests.rs`
   satisfies the hot-path pragma guard vacuously. That phase is order-independent and shares no file
   with the other five.
