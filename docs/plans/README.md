...
| [0094](0094-the-two-doc-gates-check-what-they-claim-to.md) | The two doc gates check what they claim to | draft | dev | **Three repairs from [0093]'s review, all in `scripts/`, nothing broken today.** The link gate skips *any* directory named `fixtures`, which silently dropped `core/tests/fixtures/README.md` and its twelve relative links (257 markdown files → 248); the claim gate never asks whether an entry *has* a verification bullet, which is ADR-0108's own Decision sentence; and the staleness advisory reads a shallow clone on CI, where `git log -1` returns the tip commit for every path. **Contends with nothing** — two script files, no Rust, no re-bless — and fits a short session next to any lane. |
...
**Its review produced [0094](0094-the-two-doc-gates-check-what-they-claim-to.md)**, which sits
outside the sequence above for the same reason 0093 did: `scripts/` only, no Rust, no file any other
plan on this roster names. Nothing is broken while it waits — all three findings are coverage a gate
claims and does not have — but the second one (a live entry with no verification bullet passes green)
means the mechanism 0093 just built does not yet hold itself up, so it wants a session before the
backlog grows again.
