...
|------|-------|--------|-------|-----------------|
| [0110](0110-the-shader-surface-stops-being-invisible.md) | The shader surface stops being invisible | draft | dev, human | **CI's `coverage` job has been red since 2026-08-16 and is the only red one** — `warp_mesh/shader.rs` is 719 lines at 0.00 %, unreached because every fixture drives the mesh from EEL2 bytecode and none carries WGSL. The arithmetic sets the scope: **746 missed lines must go and the two files in scope hold 990**, so the margin is thin and the floor stays at 91 by decision rather than by drift. |
...
| [0109](0109-the-milkdrop-import-gets-its-geometry-back.md) | The MilkDrop import gets its geometry back | draft | dev, human | **Succeeds [0108](done/0108-the-milkdrop-import-gets-its-tone-back.md), whose look gate closed Plan 0100's HDR claim negatively and found four defects that plan never touched.** Ordered by **certainty, not size**: Phase 1 carries the sign through the rate conversion so a negative `sx` mirrors instead of collapsing (`pow(max(v, 1e-4), dt)` deletes MilkDrop's standard mirror idiom — **363 corpus files, 3.5 %**); Phase 2 removes `time * 0.05` from the mode 6/7 waveform angle, now convicted rather than suspected; Phase 3 builds the **video-echo stage** the engine has never had (2.4 % of the corpus, but *Songflower* is unrecognisable without it). **Phase 4 is the wash — the dominant defect, cause unknown, with a stop condition**, and it is fourth precisely because it is the only phase that might not converge. Two dead hypotheses are recorded in it so nobody re-runs them. Contends with nothing. |
