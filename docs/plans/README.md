...
written**, and [0088] arrived from a user request after the sweep. They are
ordered above roughly smallest-and-most-urgent first; see the
...
3. **[0085]** — before R0 is designed, which is its only hard constraint. Touches `standalone/`;
   its contention with [0083] for `main.rs` is discharged (above), so it needs no sequencing.
...
- **It gates nothing and is gated by nothing**, so it takes any free session. File contention is
  ...
  one assertion in `core/tests/preset.rs`. The overlap with [0084] was nominal — both touch `scripts/`
  and `core/tests/`, but different files in each — and is **moot since [0084] closed 2026-08-13**.
