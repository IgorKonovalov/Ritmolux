...
| 3 | [0067] | The curation route. `reactivity.rs` now runs only in `coverage` (ADR-0073), so a break there shows up in one job. |
| 4 | [0046] | Touches the attractor's feedback path, cheapest once that file has settled. |
| 5 | [0076] | Unblocked — [0071] has landed (shared files: `render/mod.rs`, `post.rs`). Best after [0046] (both touch the chain). Before [0075]'s cohorts by preference, so the brief can assign layered worlds. |
| 6 | [0075] | **Last by design** (ADR-0089): the fresh library is authored once, against the engine [0046]/[0071]/[0076] finish ([0064] has landed) and through the gate [0067] makes trustworthy. Its Phases 1-3 are the exception — ungated instrument/doc fixes, take them whenever a session has room. |
