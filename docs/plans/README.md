...
| [0061](0061-the-build-stops-paying-for-what-it-is-not-building.md) | The build stops paying for what it is not building, and the oversized modules come apart | **approved 2026-08-08** — ready for `dev`, sequenced **last** by the user's own instruction. Thirteen phases from a whole-tree maintainability audit: dev-profile build cost (1, 1b), the `core-cabi` extraction ([ADR-0072](../adrs/0072-the-c-abi-ships-from-its-own-crate.md)), the CI double payment (2b, 2c, [ADR-0073](../adrs/0073-the-windows-ci-critical-path.md)), the tree-wide test-module exodus (2d), the `Renderer` and `particles/` carve-outs (3, 6), and this index itself (7b). **Phase 4b's scoping half already landed out of sequence** as `1c55476`, which opens a `--report` coverage gap until Phase 4. Every file measurement was re-taken 2026-08-08; the build and CI numbers are still 2026-08-04 snapshots, so **re-measure before acting**. Moves **no pixels** — a golden baseline diff is a phase failure | dev, human |
