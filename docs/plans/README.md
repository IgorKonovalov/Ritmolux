...
**Next free number: 0119** (ADRs are a separate sequence — next free there is **0129**.)
...
| [0114](0114-the-line-stroke-reads-as-a-drawn-line.md) | The line stroke reads as a drawn line | approved | dev, human | **Takes priority over [0087](0087-the-line-renderer-draws-a-curve.md), which is parked at its Phase 4 by user decision 2026-08-25.** 0087's look gate returned *“circles looks fine but blurred”* — the first half closed that plan's question, the second half is this one. The stroke is a quadratic falloff across the **whole** half-width, so a 14 px line is a 4 px spine in a 10 px gradient; measured with bloom, trails and `glow` all unbound, so it is the stroke and not a post stage. **Nothing regressed** — it is [ADR-0056](../adrs/0056-additive-scenes-emit-premultiplied-alpha.md) working as specified, and `glow`/`thickness`/bloom provably cannot reach it. Carries [ADR-0124](../adrs/0124-the-line-stroke-carries-a-solid-core-and-a-pixel-wide-edge.md) (proposed): a `softness` param plus an `fwidth`-derived one-pixel edge, where `1.0` is today's fragment byte-for-byte. **Phase 4 is a human look gate that picks the default** — deliberately, on the 0087 precedent. Reaches **all five shipped line presets** and moves every line baseline once. Contends with 0087 on `core/src/render/scenes/lines/`, which is why that one is parked rather than run beside it. **Amended 2026-08-26 to nine phases:** the fragment has a **fifth consumer**, `warp_mesh` via `draw_split`, and one `draw_all` writes one uniform so it cannot abstain. It is **pinned** at the pre-0114 profile — it answers to `foo_vis_milk2`, not to Phase 4 — and **new Phases 7-9** judge it against that reference. Its three goldens carry no line geometry, so Phase 5 stays lines-only; the finding is that **nothing in the corpus shades a `warp_mesh` stroke**, which Phase 9 fixes. |
| [0087](0087-the-line-renderer-draws-a-curve.md) | The line renderer draws a curve | in-progress | dev, human | **PARKED at Phase 4 by user decision 2026-08-25, pending [0114](0114-the-line-stroke-reads-as-a-drawn-line.md).** Phases 1, 1b, 2, 3 and 4 landed and were reviewed; 5, 6 and 7 are unstarted and Phase 5 stays green-lit. The branch is green and unpushed. Both gates cleared: Phase 3's stop condition did **not** fire (a 40-member arc ring costs +0.036 ms sparse / +0.270 ms dense against a 16.67 ms budget, on a discrete laptop GPU — that reading can convict and cannot acquit the NFR §1 iGPU floor), and Phase 4's look gate returned *“circles looks fine but blurred”*, which green-lights **Phase 5, the biarc chain**. Two obligations this row carried turned out **not to be owed**: no baseline moved (no shipped preset declares a `rings` roster, so the re-bless never existed — and 28 was already stale, the count is 18 in `golden.rs` of 33 files), and the arc pipeline **shares** the segment pipelines' bind layout, so ADR-0058 has no new entry to make. Its Phase 4 note — that the stroke reads as a thin spine inside a wide gradient — is [ADR-0056](../adrs/0056-additive-scenes-emit-premultiplied-alpha.md)'s falloff working as specified and is a **separate** question, not this plan's. Watch [ADR-0037](../adrs/0037-internal-grid-is-a-resolution-not-a-shape.md): the arc control runs at 4:3 and is verified to bite. |
...
| [0113](0113-the-engine-paints-a-canvas.md) | The engine paints a canvas | in-progress | dev, human | **A twelfth system, `shape_collage` — the engine's first *graphic* world rather than a luminous one, from the user's Russian-avant-garde reference set.** Carries [ADR-0123](../adrs/0123-a-flat-graphic-scene-paints-its-own-paper-and-composites-opaque-elements-in-one-pass.md) (proposed), whose finding is that this needs **no composite change at all**: a fullscreen scene emitting alpha 1 already holds the backdrop out (measured, Plan 0091 Phase 1), the tonemap is the identity below `KNEE = 0.6` so a flat fill survives the post chain unshaded, and bloom's threshold sits above that knee so hard edges come free. **Phase 3 is a stop gate** — the draw path is O(elements) per pixel and the bounding-box reject saves the body, not the loop step, so Phase 2 measures before Phase 4 builds; it can route the plan to ADR-0123's Alternative A or B. **Phase 5 is a second human gate**, picking the layout grammar from rendered samples. Advances [design-backlog 0069](../design-backlog.md) without closing it: occlusion works *within* a scene, cross-scene ordering still does not exist. **Phases 1-8 landed; [0116](done/0116-the-sanity-lens-finds-the-ground.md) merged in 2026-08-26 and unblocked Phase 6b, which is what remains.** Against the derived ground the two shipped canvases read `coverage` 0.2677 / 0.3028, not the `1.0000` the Phase 1 arm assumed, so both sit under a `0.50` floor and clear only on the structural rescue: the arm re-derives to **0.13**, half the family minimum. Watch [ADR-0037](../adrs/0037-internal-grid-is-a-resolution-not-a-shape.md) — this computes screen-destined geometry, and Phase 1's 1280x800 check is the only thing that discriminates it. |
