...
| [0207](0207-the-commitments-get-their-instruments.md) | The commitments get their instruments | draft | dev, human | ADR-0231 + 0232 (proposed): the exe cap re-derived and a build that prints it; an ADVISORY frame-cost column; the Floor read on a named machine. Phase 1 sets the constant. Closes backlog 0257. |
| [0206](0206-the-browser-shows-the-look.md) | The browser shows the look | draft | dev | ADR-0230 (proposed): a low-priority subprocess of the player renders each still, cached by name + file stamp; a pane beside the list, never a grid. After 0205 - shares the overlay. Exe is already over cap, backlog 0257. |
