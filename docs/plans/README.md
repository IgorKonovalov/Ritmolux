...
**Next free number: 0120** (ADRs are a separate sequence — next free there is **0129**.)
...
|------|-------|--------|-------|-----------------|
| [0118](0118-the-comments-stop-narrating-the-plans-that-wrote-them.md) | The comments stop narrating the plans that wrote them | draft | dev | Carries [ADR-0128](../adrs/0127-a-comment-carries-the-mechanism-and-the-decision-record-stays-in-docs.md) (proposed) and **retires [design-backlog 0129](../design-backlog.md)**, which proposed guarding the 89 `.rs` links this deletes. **Phases 6-7 are 3,850 lines of judgement under one rule**: delete a sentence only if the ADR it cites already holds the fact, else keep it and log it for the architect to promote — a log with nothing kept is the failure signal. **Phase 7 is separable**, its 34 files being where `plan-0113`, `plan-0087` and `plan-0116` are live — `core/tests/sanity.rs` is in it, being both the sweep's heaviest file and one 0116 rewrites. Changes no behaviour, so a moved golden means something went wrong. |
| [0119](0119-the-flatness-gate-gets-its-second-term.md) | The flatness gate gets its second term | in-progress | dev | **Phase 2 decided 2026-08-26: `boundary` ships at `MIN_BOUNDARY_DENSITY = 0.31`, not the tiled statistic** — carries [ADR-0130](../adrs/0130-the-structural-term-is-boundary-density-and-conditioning-the-population-is-what-made-it-work.md), and [ADR-0129](../adrs/0129-the-structural-term-is-measured-at-composition-scale-not-pixel-scale.md) keeps its corrected stop condition but loses its Decision to a dated `Outcome`. Phases 3-5 remain: the conjunction, shipping `fragment_tiledmono` (whose frame becomes a frozen calibration anchor — do not re-tune it), and the docs. |
| [0114](0114-the-line-stroke-reads-as-a-drawn-line.md) | The line stroke reads as a drawn line | in-progress | human, dev | **The `dev` arm is done, reviewed and merged (2026-08-26): phases 1-5 and 7-9, carrying [ADR-0124](../adrs/0124-the-line-stroke-carries-a-solid-core-and-a-pixel-wide-edge.md), now accepted with an `Outcome`.** The line fragment gained an authorable `softness`; the look gate set the default to **0.25** and `warp_mesh` is **pinned at 1.0** under its own named constant, judged against `foo_vis_milk2` rather than that gate (Phase 8 kept the pin, which the plan names as a legitimate close). Phase 9 added `warp_mesh_stroke` — the first baseline in the corpus that can see this fragment at all — and it is **verified to convict**: driving the pin `1.0 -> 0.0` moves it to mean 0.0336 / outlier 162 while all twenty others read 0.0000. **Two phases outstanding, neither gating the other.** **Phase 6** is a `preset-author` sitting: the five presets tuned against the old profile still bind nothing, and Phase 4's verdict already names what they want — `curve_ionwake` **0**, the Maurer roses (`curve_nightbloom`, `fragment_vitrail`'s line layer) **1.0**, `lsystem_vellum` **0.25** — so until it runs, two shipped presets render at a stroke that gate said is wrong for them. **Phase 10** is the Mode 4 review's one major plus two minors, all in test surface. **Re-shoot `docs/images/gallery/`'s four line renders after Phase 6, not before.** |
| [0087](0087-the-line-renderer-draws-a-curve.md) | The line renderer draws a curve | in-progress | dev, human | **PARKED at Phase 4 by user decision 2026-08-25, pending [0114](0114-the-line-stroke-reads-as-a-drawn-line.md).** Phases 1, 1b, 2, 3 and 4 landed and were reviewed; 5, 6 and 7 are unstarted and Phase 5 stays green-lit. **Its code is on `main` as of 2026-08-26** — [0114](0114-the-line-stroke-reads-as-a-drawn-line.md) was branched off this lane (its Phase 2 needs the arc fragment) and carried phases 1-4 in on its own merge, so the arc primitive ships and the remaining phases resume from `main` rather than from this branch. Both gates cleared: Phase 3's stop condition did **not** fire (a 40-member arc ring costs +0.036 ms sparse / +0.270 ms dense against a 16.67 ms budget, on a discrete laptop GPU — that reading can convict and cannot acquit the NFR §1 iGPU floor), and Phase 4's look gate returned *“circles looks fine but blurred”*, which green-lights **Phase 5, the biarc chain**. Two obligations this row carried turned out **not to be owed**: no baseline moved (no shipped preset declares a `rings` roster, so the re-bless never existed — and 28 was already stale, the count is 18 in `golden.rs` of 33 files), and the arc pipeline **shares** the segment pipelines' bind layout, so ADR-0058 has no new entry to make. Its Phase 4 note — that the stroke reads as a thin spine inside a wide gradient — is [ADR-0056](../adrs/0056-additive-scenes-emit-premultiplied-alpha.md)'s falloff working as specified and is a **separate** question, not this plan's. Watch [ADR-0037](../adrs/0037-internal-grid-is-a-resolution-not-a-shape.md): the arc control runs at 4:3 and is verified to bite. |
...
- **2026-08-26: now owed, by a different mechanism, and deliberately deferred once more.**
  [0114](0114-the-line-stroke-reads-as-a-drawn-line.md) moved the line stroke's default `softness`
  from `1.0` to `0.25`, so **all four** line-scene renders — `parametric_curve.png`,
  `lsystem.png`, `star_pattern.png` and `spectrum.png` — show a stroke the engine no longer
  draws. The note above is not wrong about arcs; it is simply no longer the only reason. Re-shoot
  **after 0114's Phase 6** retunes the five line presets, or the same four images are captured
  twice. `scripts/docs-shots.mjs` re-shoots them and
  [ADR-0100](../adrs/0100-documentation-images-are-committed-headless-renders.md) deliberately has
  no CI check that would notice, so nothing fails in the meantime and nothing will remind anyone.
