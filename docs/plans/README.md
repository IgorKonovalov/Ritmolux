...
|------|-------|--------|-------|-----------------|
| [0117](0117-the-downbeat-log-sees-the-counter-it-folds-over.md) | The downbeat log sees the counter it folds over | draft | dev | **`main` is red and unpushed at `c0869e6`** — Plan 0095 moved the fold onto the grid's count and `--downbeat-log` still writes `beat_index`, so its `beat` column and its `s0`..`s3`/`best` columns are in different spaces. Two appended columns and a repointed test. Also the instrument [ADR-0109](../adrs/0109-the-beat-clock-counts-onsets-not-beats.md)'s `Outcome` says is missing. |
