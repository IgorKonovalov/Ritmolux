...
| [0119](0119-the-flatness-gate-gets-its-second-term.md) | The flatness gate gets its second term | approved | dev, human | Carries [ADR-0129](../adrs/0129-the-structural-term-is-measured-at-composition-scale-not-pixel-scale.md) (proposed) and ships `presets/pending/fragment_tiledmono.toml` or explains why it stays held. **Phase 2 is a stop gate that has already fired twice** — ADR-0126's diagnosis died at Plan 0116 Phase 1, ADR-0128's mechanism at Phase 8. The finding behind this attempt: all three dead candidates measure **one axis**, pixel-scale geometry of the lit mask, which a particle blot has more of than the library's quietest presets; and the stop condition was judged over the whole library when a conjunction's second term only ever sees frames that failed the first. Conditioned correctly the calibration population has **two** members. |
