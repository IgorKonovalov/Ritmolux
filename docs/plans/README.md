...
- **Plan [0090] Phase 5 — the two emitter worlds, and the verdict on the pop** (2026-08-15). The
  plan is `done` and all four `dev` phases landed; this is the phase the plan exists for. Two looks
  the engine can now reach and could not before: **a quiet drifting field** (the sky
  [backlog 0068](../design-backlog-archive.md#0068--a-swarm-mark-has-no-per-mark-variation-so-the-only-scene-that-can-hold-a-starfield-cannot-make-one-twinkle)
  measured the emitter for and could not get past the gates) and **a point fountain or off-centre
  jet** (`source_width = 0` plus `pan_x`, the ask `presets/README.md` used to route to engine
  feedback). Two questions ride it that no test can answer, and both are recorded as *accepted
  prices* in [ADR-0104](../adrs/0104-the-emitters-source-is-authorable-geometry.md) on the strength
  of arguments nobody has looked at: **does `spawn_fade` actually hide the pop** of an inside-frame
  source, and **does a prewarmed world switch in badly** (it arrives fully populated in one frame —
  right for a sky, wrong for a cascade). The verdicts belong in the world's header, the way the
  fold-edge verdicts are recorded. Content-lane work; it joins the standing sitting on this family.
  **One number to take in with you:** the slow draft measured for Phase 3 passed `sanity` and
  `animation` at `prewarm = 1` and came up **0.0195 against a 0.02 reactivity floor** — 97.5 % of
  it, on a draft nobody tuned for that gate. That reads as content tuning; if it turns out not to
  be, it is a finding for [ADR-0091](../adrs/0091-the-animation-gate-scores-motion-against-the-figures-footprint.md)
