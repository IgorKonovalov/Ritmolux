...
|------|-------|--------|-------|-----------------|
| [0118](0118-the-comments-stop-narrating-the-plans-that-wrote-them.md) | The comments stop narrating the plans that wrote them | draft | dev | Carries [ADR-0127](../adrs/0127-a-comment-carries-the-mechanism-and-the-decision-record-stays-in-docs.md) (proposed) and **retires [design-backlog 0129](../design-backlog.md)**, which proposed guarding the 89 `.rs` links this deletes. **Phases 6-7 are 3,850 lines of judgement under one rule**: delete a sentence only if the ADR it cites already holds the fact, else keep it and log it for the architect to promote — a log with nothing kept is the failure signal. **Phase 7 is separable**, its 31 files being where `plan-0113` and `plan-0087` are live; changes no behaviour, so a moved golden means something went wrong. |
