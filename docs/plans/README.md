...
**Next free number: 0122** (ADRs are a separate sequence — next free there is **0135**.)
...
| [0121](0121-a-rate-an-ink-edge-and-a-motion-reading.md) | A rate, an ink edge, and a motion reading | approved | dev, human | **The mono cohort's three API-feedback items, taken together — backlog 0137/0138/0139 plus the 0131 re-raise.** Carries [ADR-0132](../adrs/0132-a-rate-parameter-integrates-a-phase.md), [ADR-0133](../adrs/0133-the-band-contour-fires-where-the-ink-changes.md) and [ADR-0134](../adrs/0134-motion-is-two-readings-and-anchoring-is-why-neither-can-be-a-threshold.md) (all proposed). **Phase 1 is the instrument and runs first** — `--report` cannot see motion rate or combined drive, which cost four live round trips on one preset, and both new columns come free from captures it already takes. **Phase 5 contends with [0098](0098-the-figure-nests-properly.md) and [0092](0092-the-engine-draws-an-authored-path.md) on `shape_field.rs`**, so run them in sequence or in separate lanes. **Phase 4 corrects `warp_speed`, which teleports on any audio binding today and has no regression evidence** because no preset binds it. **Phase 6 is a `preset-author` session** and is the only evidence the walls came down. |
...
1. **[0098](0098-the-figure-nests-properly.md)** — ~~after [0087]~~, **discharged: 0087 closed
   2026-08-27**. Both edit
...
2. **[0092](0092-the-engine-draws-an-authored-path.md)** — after 0098 and **not beside it**: both
   ...
   [0087]'s outcome, which **now exists**: the arc primitive shipped and ADR-0098's Alternative C
   was not taken, so a polyline distance field is not the only route.
4. **[0104]** — ~~once [0087] and~~ once 0098 has resolved. **[0087] closed 2026-08-27, so its Phase 4
   is unblocked**: `star_pattern` can be authored on the arc primitive now, and
   `star_mandala_bordered` is the worked example of what that surface reaches. Every phase is a `preset-author` session in `presets/`.
