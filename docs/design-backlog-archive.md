...
- [0003 — Fixed internal resolutions: RD at 256x256, trails and kaleidoscope at 1280x720](#0003--fixed-internal-resolutions-rd-at-256x256-trails-and-kaleidoscope-at-1280x720)
- [0004 — `zoom`/`pan_*` on reaction-diffusion smear the edge: a toroidal sim behind a clamped sampler](#0004--zoompan_-on-reaction-diffusion-smear-the-edge-a-toroidal-sim-behind-a-clamped-sampler)
- [0005 — No bloom / glow / halo stage](#0005--no-bloom--glow--halo-stage)
- [0006 — `[smoothing]` is a one-pole low-pass: no attack/release split, no S-curve](#0006--smoothing-is-a-one-pole-low-pass-no-attackrelease-split-no-s-curve)
- [~~0007 — `star_pattern` reads as a hollow ring, and discrete `variant` cannot be blended~~](#0007--star_pattern-reads-as-a-hollow-ring-and-discrete-variant-cannot-be-blended)
- [0008 — `shot` harness gaps that cost the content lane real iterations](#0008--shot-harness-gaps-that-cost-the-content-lane-real-iterations)
- [Entries 0010-0014 — the 2026-07-26 `preset-author` API-feedback batch (second, post-Plan-0033)](#entries-0010-0014--the-2026-07-26-preset-author-api-feedback-batch-second-post-plan-0033)
- [0010 — the kaleidoscope fold samples outside its source rectangle and clamps, leaving edge debris](#0010--the-kaleidoscope-fold-samples-outside-its-source-rectangle-and-clamps-leaving-edge-debris)
- [0011 — the kaleidoscope fold axis is screen-centred, so `pan_*` and `kaleido_*` are mutually exclusive](#0011--the-kaleidoscope-fold-axis-is-screen-centred-so-pan_-and-kaleido_-are-mutually-exclusive)
- [0012 — `--report`'s `cover` metric structurally penalises inverted-polarity (ink) presets](#0012----reports-cover-metric-structurally-penalises-inverted-polarity-ink-presets)
- [0013 — no synthetic signal has transients, so a `[smoothing]` change cannot be verified at all](#0013--no-synthetic-signal-has-transients-so-a-smoothing-change-cannot-be-verified-at-all)
- [0014 — the line scenes' cosine `hue` ramp is not a hue wheel, and nothing documents it](#0014--the-line-scenes-cosine-hue-ramp-is-not-a-hue-wheel-and-nothing-documents-it)
- [Entries 0015-0019 — the 2026-07-27 batch (third, post-Plan-0034)](#entries-0015-0019--the-2026-07-27-batch-third-post-plan-0034)
- [0015 — the band axis is half linear, and it is undecided whether that is a defect](#0015--the-band-axis-is-half-linear-and-it-is-undecided-whether-that-is-a-defect)
- [0016 — the `spectrum` readout has no width control, and density makes it worse](#0016--the-spectrum-readout-has-no-width-control-and-density-makes-it-worse)
- [0017 — `[spectrum]` has no level curve, and the grammar has no `log`, so a dB readout is impossible](#0017--spectrum-has-no-level-curve-and-the-grammar-has-no-log-so-a-db-readout-is-impossible)
- [0018 — `BASELINE_Y` is a constant, so `mirror_reflect` throws the copy to the top of the frame](#0018--baseline_y-is-a-constant-so-mirror_reflect-throws-the-copy-to-the-top-of-the-frame)
- [0019 — `glow` is unreachable from a preset on all four line scenes](#0019--glow-is-unreachable-from-a-preset-on-all-four-line-scenes)
- [Entry 0020 — from the Plan 0037 measurement phase](#entry-0020--from-the-plan-0037-measurement-phase)
- [0020 — the shipped library is gained against stimuli 6-100x hotter than real music](#0020--the-shipped-library-is-gained-against-stimuli-6-100x-hotter-than-real-music)
- [0022 — `--report`'s reactivity columns are structurally blind to a level `curve`](#0022----reports-reactivity-columns-are-structurally-blind-to-a-level-curve)
- [0023 — `LineRenderer` has no line joins, so every direction change leaves a notch](#0023--linerenderer-has-no-line-joins-so-every-direction-change-leaves-a-notch)
- [0024 — the star rosette is a closed chain, and half its joints are still unjoined](#0024--the-star-rosette-is-a-closed-chain-and-half-its-joints-are-still-unjoined)
- [Entries 0025-0027 — the 2026-07-28 `preset-author` batch (fourth), from the full library retune](#entries-0025-0027--the-2026-07-28-preset-author-batch-fourth-from-the-full-library-retune)
- [~~0025 — `swarm` cannot express a flock: no depth, no cohesion, and its field frequency is a constant~~](#0025--swarm-cannot-express-a-flock-no-depth-no-cohesion-and-its-field-frequency-is-a-constant)
- [~~0026 — `lsystem` has no per-segment colour, and the asymmetry with `spectrum` looks unintentional~~](#0026--lsystem-has-no-per-segment-colour-and-the-asymmetry-with-spectrum-looks-unintentional)
- [0027 — two engine behaviours that are correct, non-obvious, and undocumented](#0027--two-engine-behaviours-that-are-correct-non-obvious-and-undocumented)
- [Entry 0028 — from the 2026-07-29 `preset-author` post-Plan-0041 library sweep](#entry-0028--from-the-2026-07-29-preset-author-post-plan-0041-library-sweep)
- [~~0028 — reachability only reports `select`/`clamp` nodes, so a bare comparison is invisible and a dead band gate can hide behind a live `tempo` one~~](#0028--reachability-only-reports-selectclamp-nodes-so-a-bare-comparison-is-invisible-and-a-dead-band-gate-can-hide-behind-a-live-tempo-one)
- [~~0029 — the swarm's toroidal wrap seam sits exactly on the frame edge, and feedback burns it into a bright bar~~](#0029--the-swarms-toroidal-wrap-seam-sits-exactly-on-the-frame-edge-and-feedback-burns-it-into-a-bright-bar)
- [0030 — the library binds audio to luminance and colour far more than to the numbers that rebuild geometry](#0030--the-library-binds-audio-to-luminance-and-colour-far-more-than-to-the-numbers-that-rebuild-geometry)
- [~~0031 — the Rich tier's 3x particle count makes the attractor reseed transient opaque, and `clifford` blows out~~](#0031--the-rich-tiers-3x-particle-count-makes-the-attractor-reseed-transient-opaque-and-clifford-blows-out)
- [~~0034 — nothing in the engine spawns, throws, ages or individuates an object~~](#0034--nothing-in-the-engine-spawns-throws-ages-or-individuates-an-object)
- [~~0035 — `presets/README.md` lists 10 expression variables; the code has 19~~](#0035--presetsreadmemd-lists-10-expression-variables-the-code-has-19)
- [0036 — the kaleidoscope stops folding the backdrop; is a folded backdrop a look worth keeping?](#0036--the-kaleidoscope-stops-folding-the-backdrop-is-a-folded-backdrop-a-look-worth-keeping)
- [~~0037 — the fold covers a disc, and on a field scene that reads as worse than the defect it replaced~~](#0037--the-fold-covers-a-disc-and-on-a-field-scene-that-reads-as-worse-than-the-defect-it-replaced)
- [~~0039 — four bind-group layouts in `core/src` are shared by pipelines that go live in the same frame, and only the tonemap's uniqueness is asserted~~](#0039--four-bind-group-layouts-in-coresrc-are-shared-by-pipelines-that-go-live-in-the-same-frame-and-only-the-tonemaps-uniqueness-is-asserted)
- [~~0041 — the line seam's lit-backdrop guard discriminates on ~5 pixels, and a stronger property is available~~](#0041--the-line-seams-lit-backdrop-guard-discriminates-on-5-pixels-and-a-stronger-property-is-available)
- [Entries 0043-0045 — the 2026-08-02 `preset-author` batch (sixth), from Plan 0048 Phase 7 (the library retune)](#entries-0043-0045--the-2026-08-02-preset-author-batch-sixth-from-plan-0048-phase-7-the-library-retune)
- [~~0043 — every reactivity instrument we have diffs against **silence**, so a binding saturated just above the noise floor reads as maximally reactive~~](#0043--every-reactivity-instrument-we-have-diffs-against-silence-so-a-binding-saturated-just-above-the-noise-floor-reads-as-maximally-reactive)
- [~~0044 — Phase 1's axis rebuild silently re-pointed every sub-crossover `bin()` probe in the library, and three preset headers plus `docs/presets.md` still teach the axis it replaced~~](#0044--phase-1s-axis-rebuild-silently-re-pointed-every-sub-crossover-bin-probe-in-the-library-and-three-preset-headers-plus-docspresetsmd-still-teach-the-axis-it-replaced)
- [~~0045 — `docs/analysis-v2-before-flags.md` has served its purpose and asks to be deleted~~](#0045--docsanalysis-v2-before-flagsmd-has-served-its-purpose-and-asks-to-be-deleted)
- [Entry 0046 — from the Plan 0048 close, and then corrected by the lane it was handed to](#entry-0046--from-the-plan-0048-close-and-then-corrected-by-the-lane-it-was-handed-to)
- [~~0046 — the retune's gain rule is direction-blind, so the six presets whose idiom *subtracts* light came out ~25 % darker~~](#0046--the-retunes-gain-rule-is-direction-blind-so-the-six-presets-whose-idiom-subtracts-light-came-out-25--darker)
- [Entries 0047-0048 — the 2026-08-03 `preset-author` batch (seventh), from the attractor ceiling pass](#entries-0047-0048--the-2026-08-03-preset-author-batch-seventh-from-the-attractor-ceiling-pass)
- [~~0047 — `Rich` triples the attractor's light with nothing normalizing for it, so the tier is **not** look-neutral, and no automated check can see the difference~~](#0047--rich-triples-the-attractors-light-with-nothing-normalizing-for-it-so-the-tier-is-not-look-neutral-and-no-automated-check-can-see-the-difference)
- [~~0048 — the `lorenz` attractor family renders as a dust cloud, and no preset key reaches the scatter~~](#0048--the-lorenz-attractor-family-renders-as-a-dust-cloud-and-no-preset-key-reaches-the-scatter)
- [0049 — the fold's residual rays got a second rejection and a shipped instance, and they are avoidable from content](#0049--the-folds-residual-rays-got-a-second-rejection-and-a-shipped-instance-and-they-are-avoidable-from-content)
- [~~0050 — the attractor reseed scatters into an axis-aligned BOX, so every reseed flashes a speckled rectangle across the frame~~](#0050--the-attractor-reseed-scatters-into-an-axis-aligned-box-so-every-reseed-flashes-a-speckled-rectangle-across-the-frame)
- [Entry 0051 — from the Plan 0054 Mode 4 review (2026-08-03)](#entry-0051--from-the-plan-0054-mode-4-review-2026-08-03)
- [0051 — `variant` can morph now, and neither shipped `star_*` preset does, because both drive it with a sawtooth](#0051--variant-can-morph-now-and-neither-shipped-star_-preset-does-because-both-drive-it-with-a-sawtooth)
- [Entry 0052 — from the Plan 0056 close (2026-08-03), found by the gate the plan built](#entry-0052--from-the-plan-0056-close-2026-08-03-found-by-the-gate-the-plan-built)
- [~~0052 — `Spectrum Ridge` has no tonal structure at all: it is the one shipped preset the new flat-frame gate convicts~~](#0052--spectrum-ridge-has-no-tonal-structure-at-all-it-is-the-one-shipped-preset-the-new-flat-frame-gate-convicts)
- [Entry 0053 — from the `preset-author` pass on 0051/0052 (2026-08-03)](#entry-0053--from-the-preset-author-pass-on-00510052-2026-08-03)
- [~~0053 — Plan 0048's retune rescaled the band GAINS but not the world-space params those bands multiply, and no instrument can see the result~~](#0053--plan-0048s-retune-rescaled-the-band-gains-but-not-the-world-space-params-those-bands-multiply-and-no-instrument-can-see-the-result)
- [Entry 0058 — from Plan 0055 Phase 4 (2026-08-04), the content half of a decision the engine has now made](#entry-0058--from-plan-0055-phase-4-2026-08-04-the-content-half-of-a-decision-the-engine-has-now-made)
- [0058 — thirteen presets bind the fold and eleven of them have not chosen an edge treatment, because until now there was nothing to choose](#0058--thirteen-presets-bind-the-fold-and-eleven-of-them-have-not-chosen-an-edge-treatment-because-until-now-there-was-nothing-to-choose)
- [Entry 0057 — from the Plan 0059 Phase 4 content pass (2026-08-04)](#entry-0057--from-the-plan-0059-phase-4-content-pass-2026-08-04)
- [~~0057 — a preset has no scene-local way to set a figure's level, so `exposure` gets used for it and two other stages disagree with that use~~](#0057--a-preset-has-no-scene-local-way-to-set-a-figures-level-so-exposure-gets-used-for-it-and-two-other-stages-disagree-with-that-use)
- [Entries 0033-0035 — the 2026-07-30 `preset-author` batch (fifth), from two figurative requests](#entries-0033-0035--the-2026-07-30-preset-author-batch-fifth-from-two-figurative-requests)
- [~~0033 — every mark the engine can draw is a round additive blob or a stroked curve, so no *object* has a shape~~ (silhouette half; the fill-and-outline half is re-filed as 0069)](#0033--every-mark-the-engine-can-draw-is-a-round-additive-blob-or-a-stroked-curve-so-no-object-has-a-shape-silhouette-half-the-fill-and-outline-half-is-re-filed-as-0069)
- [Entry 0054 — from the Plan 0058 close (2026-08-03), and the plan's own measurement is the argument](#entry-0054--from-the-plan-0058-close-2026-08-03-and-the-plans-own-measurement-is-the-argument)
- [0054 — pixel coverage cannot see a figure whose *tips* leave the frame, and an in-frame geometry fraction is the successor](#0054--pixel-coverage-cannot-see-a-figure-whose-tips-leave-the-frame-and-an-in-frame-geometry-fraction-is-the-successor)
- [0040 — additive light occludes by geometry, so a dim figure over a lit backdrop reads as dark speckle](#0040--additive-light-occludes-by-geometry-so-a-dim-figure-over-a-lit-backdrop-reads-as-dark-speckle)
- [0009 — the `animation.rs` gate penalizes two legitimate designs (informational)](#0009--the-animationrs-gate-penalizes-two-legitimate-designs-informational)
- [0055 — the attractor's shape vocabulary is "breathe and bend", and the reference figures ask for more](#0055--the-attractors-shape-vocabulary-is-breathe-and-bend-and-the-reference-figures-ask-for-more)
- [Entry 0056 — from the Plan 0050 close (2026-08-04), found while clearing a stale preset cache](#entry-0056--from-the-plan-0050-close-2026-08-04-found-while-clearing-a-stale-preset-cache)
- [0056 — a user-authored preset has been living outside the repo for six weeks, and it is a curation candidate the boundary has no route for](#0056--a-user-authored-preset-has-been-living-outside-the-repo-for-six-weeks-and-it-is-a-curation-candidate-the-boundary-has-no-route-for)
- [Entries 0059-0060 — from the `preset-author` handoff of 2026-08-04, after Squall and the fold-edge pass](#entries-0059-0060--from-the-preset-author-handoff-of-2026-08-04-after-squall-and-the-fold-edge-pass)
- [0059 — the backdrop is the one surface left that does not colour through the shared palette, and nothing says so](#0059--the-backdrop-is-the-one-surface-left-that-does-not-colour-through-the-shared-palette-and-nothing-says-so)
- [0060 — an engine fix leaves its preset-side workarounds standing, and only a header comment remembers them](#0060--an-engine-fix-leaves-its-preset-side-workarounds-standing-and-only-a-header-comment-remembers-them)
- [Entries 0061-0063 — from the Plan 0063 Phase 5 content pass (2026-08-04)](#entries-0061-0063--from-the-plan-0063-phase-5-content-pass-2026-08-04)
- [0061 — `perspective` moves the figure far more than it enlarges it, so the documented way to recover the framing does not work](#0061--perspective-moves-the-figure-far-more-than-it-enlarges-it-so-the-documented-way-to-recover-the-framing-does-not-work)
- [0062 — `depth_hue` is a *lightness* cue on a lightness ramp, it wraps at the ends, and it is structurally dead under `ink_amount`](#0062--depth_hue-is-a-lightness-cue-on-a-lightness-ramp-it-wraps-at-the-ends-and-it-is-structurally-dead-under-ink_amount)
- [0063 — `spin`'s usable ceiling is set by `fade`, not by taste, and the pair is undocumented](#0063--spins-usable-ceiling-is-set-by-fade-not-by-taste-and-the-pair-is-undocumented)
- [Entries 0064-0066 — from the Plan 0062 Phase 7 content pass (2026-08-04)](#entries-0064-0066--from-the-plan-0062-phase-7-content-pass-2026-08-04)
- [0064 — an IFS preset switch shows a hard-edged rectangle of noise for two thirds of a second, which is the artifact ADR-0066 already removed once](#0064--an-ifs-preset-switch-shows-a-hard-edged-rectangle-of-noise-for-two-thirds-of-a-second-which-is-the-artifact-adr-0066-already-removed-once)
- [~~0065 — `morph` is a travel knob whose visible rate is steepest near zero, and nothing says so~~](#0065--morph-is-a-travel-knob-whose-visible-rate-is-steepest-near-zero-and-nothing-says-so)
- [~~0066 — the IFS figures are STILL, so the library's conventions about drift rates are wrong for them~~](#0066--the-ifs-figures-are-still-so-the-librarys-conventions-about-drift-rates-are-wrong-for-them)
- [0067 — `depth_fade` is a uniform dimmer on every flat family, where the other two depth cues are exact no-ops](#0067--depth_fade-is-a-uniform-dimmer-on-every-flat-family-where-the-other-two-depth-cues-are-exact-no-ops)
- [Entry 0070 — from the Plan 0069 close (2026-08-06), and it is what that plan turned out not to buy](#entry-0070--from-the-plan-0069-close-2026-08-06-and-it-is-what-that-plan-turned-out-not-to-buy)
- [0070 — the in-frame geometry fraction cannot gate new content, and the number it computes for every line preset is not in the author's report](#0070--the-in-frame-geometry-fraction-cannot-gate-new-content-and-the-number-it-computes-for-every-line-preset-is-not-in-the-authors-report)
- [0072 — `sanity.rs`'s coverage floor forces dense thin-stroke line scenes into washed-out tuning, and it is measuring the halo](#0072--sanityrss-coverage-floor-forces-dense-thin-stroke-line-scenes-into-washed-out-tuning-and-it-is-measuring-the-halo)
- [~~Entry 0074 — from Plan 0073 Phase 6 (2026-08-06), the content pass on the two IFS colour channels~~ — BUILT](#entry-0074--from-plan-0073-phase-6-2026-08-06-the-content-pass-on-the-two-ifs-colour-channels--built)
- [~~0074 — the age channel has nothing spatial to colour, because the emergence ramp hides the only steps where age correlates with position~~](#0074--the-age-channel-has-nothing-spatial-to-colour-because-the-emergence-ramp-hides-the-only-steps-where-age-correlates-with-position)
- [~~0076 — the operator docs describe a fern tuning that the shipped fern does not carry~~ — REPAIRED](#0076--the-operator-docs-describe-a-fern-tuning-that-the-shipped-fern-does-not-carry--repaired)
- [0084 — the ink stage has no contrast lever, and three worlds in two cohorts paid for it](#0084--the-ink-stage-has-no-contrast-lever-and-three-worlds-in-two-cohorts-paid-for-it)
- [0085 — `swarm` has no `reseed`, so a flow-field pile-up has no recovery lever](#0085--swarm-has-no-reseed-so-a-flow-field-pile-up-has-no-recovery-lever)
- [0088 — `shot --report`'s band columns cannot see reactivity spent on bloom](#0088--shot---reports-band-columns-cannot-see-reactivity-spent-on-bloom)
- [0091 — there is no static, screen-anchored, oriented gradient, so a horizon cannot be drawn](#0091--there-is-no-static-screen-anchored-oriented-gradient-so-a-horizon-cannot-be-drawn)
- [Entry 0077 — from the Plan 0061 close (2026-08-08), and it is a gate's blind spot rather than a preset gap](#entry-0077--from-the-plan-0061-close-2026-08-08-and-it-is-a-gates-blind-spot-rather-than-a-preset-gap)
- [0077 — the doc-link gate is blind to reference-style links, so 85 of them rendered as bracket noise behind a green check](#0077--the-doc-link-gate-is-blind-to-reference-style-links-so-85-of-them-rendered-as-bracket-noise-behind-a-green-check)
- [0080 — the reactivity gate pays 1.8x to render frames it throws away, because warm-up and measurement share one capture path](#0080--the-reactivity-gate-pays-18x-to-render-frames-it-throws-away-because-warm-up-and-measurement-share-one-capture-path)
- [~~0090 — the Mac build's capture verdict is stderr-only, so a Finder-launched tester cannot tell us why it hears nothing~~](#0090--the-mac-builds-capture-verdict-is-stderr-only-so-a-finder-launched-tester-cannot-tell-us-why-it-hears-nothing)
- [~~0078 — `kaleido_tile` is a discrete quantity that is not quantized, so it is the one term of the composed map an author cannot bind~~ — FALSIFIED](#0078--kaleido_tile-is-a-discrete-quantity-that-is-not-quantized-so-it-is-the-one-term-of-the-composed-map-an-author-cannot-bind--falsified)
- [0081 — the house gain rule lives only in preset headers *(the first half is FALSIFIED; the exception class survives)*](#0081--the-house-gain-rule-lives-only-in-preset-headers-the-first-half-is-falsified-the-exception-class-survives)
- [0089 — the dragon overruns the frame corner at the default view, and `FRAME_FILL = 0.88` promises it cannot](#0089--the-dragon-overruns-the-frame-corner-at-the-default-view-and-frame_fill--088-promises-it-cannot)
- [0082 — the quality governor reads `frame_ms_p99`, and a preset switch spikes p99 to 25 ms while nothing is dropped](#0082--the-quality-governor-reads-frame_ms_p99-and-a-preset-switch-spikes-p99-to-25-ms-while-nothing-is-dropped)
- [0086 — no capture path reaches the minutes-long horizon, so a slow-accumulation failure is invisible to every instrument](#0086--no-capture-path-reaches-the-minutes-long-horizon-so-a-slow-accumulation-failure-is-invisible-to-every-instrument)
- [0083 — RSS grew 385 to 663 MB over three minutes of preset switching, and there is no no-feedback control to compare it against](#0083--rss-grew-385-to-663-mb-over-three-minutes-of-preset-switching-and-there-is-no-no-feedback-control-to-compare-it-against)
- [0068 — a swarm mark has no per-mark variation, so the only scene that can hold a starfield cannot make one twinkle](#0068--a-swarm-mark-has-no-per-mark-variation-so-the-only-scene-that-can-hold-a-starfield-cannot-make-one-twinkle)
- [0093 — the headless capture path dies past a few thousand frames, so the horizon cannot reach its own headline length](#0093--the-headless-capture-path-dies-past-a-few-thousand-frames-so-the-horizon-cannot-reach-its-own-headline-length)
- [0106 — converted MilkDrop presets wash out or invert: the float feedback field never truncates](#0106--converted-milkdrop-presets-wash-out-or-invert-the-float-feedback-field-never-truncates)
- [0107 — the MilkDrop draw layer misplaces figures, and two warp-path defects mirror or unfold the frame](#0107--the-milkdrop-draw-layer-misplaces-figures-and-two-warp-path-defects-mirror-or-unfold-the-frame)
- [0114 — a negative scale is clamped away, so MilkDrop's standard mirror idiom collapses instead of mirroring](#0114--a-negative-scale-is-clamped-away-so-milkdrops-standard-mirror-idiom-collapses-instead-of-mirroring)
- [0115 — there is no video-echo stage, and one preset in seven is unrecognisable without it](#0115--there-is-no-video-echo-stage-and-one-preset-in-seven-is-unrecognisable-without-it)
- [0116 — the mode 6/7 waveform rotates a full turn every two minutes, and the reference's does not](#0116--the-mode-67-waveform-rotates-a-full-turn-every-two-minutes-and-the-references-does-not)
- [0121 — a bundle that never names `decay` reads MilkDrop's per-frame default as a per-second one](#0121--a-bundle-that-never-names-decay-reads-milkdrops-per-frame-default-as-a-per-second-one)
- [0098 — `thickness` below 0.167 is a dead zone on every line scene: all values render identically and nothing says so](#0098--thickness-below-0167-is-a-dead-zone-on-every-line-scene-all-values-render-identically-and-nothing-says-so)
- [0071 — the scalloped boundary was chosen as a real curve primitive, and the engine has none](#0071--the-scalloped-boundary-was-chosen-as-a-real-curve-primitive-and-the-engine-has-none)
- [0073 — motif outlines show their vertices, and a sampled polyline does not read as a curve](#0073--motif-outlines-show-their-vertices-and-a-sampled-polyline-does-not-read-as-a-curve)
- [0131 — `shot --report` truncates preset names to 14 characters, and the library now has its first collision](#0131--shot---report-truncates-preset-names-to-14-characters-and-the-library-now-has-its-first-collision)
- [0137 — `fragment_field` has three hardcoded animation rates and no parameter behind any of them, so "slow it down" and "flatten it out" are one knob](#0137--fragment_field-has-three-hardcoded-animation-rates-and-no-parameter-behind-any-of-them-so-slow-it-down-and-flatten-it-out-are-one-knob)
- [0138 — `palette_contour` keys on the band grid and never reads the LUT, so on any limited-ink palette its only usable value is zero](#0138--palette_contour-keys-on-the-band-grid-and-never-reads-the-lut-so-on-any-limited-ink-palette-its-only-usable-value-is-zero)
- [0139 — nothing in the harness measures motion RATE or the silent-to-driven difference, so a preset can pass every gate and still be unwatchable in either direction](#0139--nothing-in-the-harness-measures-motion-rate-or-the-silent-to-driven-difference-so-a-preset-can-pass-every-gate-and-still-be-unwatchable-in-either-direction)
- [0096 — `shape_field` draws offset contours, and the reference construction everyone reaches for is scaled copies](#0096--shape_field-draws-offset-contours-and-the-reference-construction-everyone-reaches-for-is-scaled-copies)
- [0097 — a curved or jittered `star` returns a NEGATIVE normalized distance at its own centre, and on `shape_field` that is a hole through the figure](#0097--a-curved-or-jittered-star-returns-a-negative-normalized-distance-at-its-own-centre-and-on-shape_field-that-is-a-hole-through-the-figure)
- [0129 — the doc-link gate walks `.md` only, so the same links inside Rust doc comments rot unwatched](#0129--the-doc-link-gate-walks-md-only-so-the-same-links-inside-rust-doc-comments-rot-unwatched)
- [0141 — ADR-0132's rule is engine-wide and its enumeration was not: `swarm`'s field clock is still `time * spin`, and two shipped presets bind that rate to the music](#0141--adr-0132s-rule-is-engine-wide-and-its-enumeration-was-not-swarms-field-clock-is-still-time--spin-and-two-shipped-presets-bind-that-rate-to-the-music)
- [0145 — the `animation` gate thresholds silent motion only, so a world whose liveliness is audio-driven fails it however alive it is](#0145--the-animation-gate-thresholds-silent-motion-only-so-a-world-whose-liveliness-is-audio-driven-fails-it-however-alive-it-is)
- [0147 — no latch: a gate cannot be armed on time and fired on the music (re-raise, second independent instance)](#0147--no-latch-a-gate-cannot-be-armed-on-time-and-fired-on-the-music-re-raise-second-independent-instance)
- [0148 — a hard-ink palette cannot reach any additive scene, so the limited-ink class is confined to 4 of 12 systems](#0148--a-hard-ink-palette-cannot-reach-any-additive-scene-so-the-limited-ink-class-is-confined-to-4-of-12-systems)
- [0122 — a mode-6 or -7 wave trace is normalized to the frame's height, so it covers `1/aspect` of its width](#0122--a-mode-6-or--7-wave-trace-is-normalized-to-the-frames-height-so-it-covers-1aspect-of-its-width)
- [0123 — the waveform is the one un-normalized analysis output, so the OS volume slider changes the picture — and the two frontends disagree](#0123--the-waveform-is-the-one-un-normalized-analysis-output-so-the-os-volume-slider-changes-the-picture--and-the-two-frontends-disagree)
- [0155 — the input-recovery settle window is counted in frames, so the flap it guards against is bounded on a 60 Hz display and unbounded on a 240 Hz one](#0155--the-input-recovery-settle-window-is-counted-in-frames-so-the-flap-it-guards-against-is-bounded-on-a-60-hz-display-and-unbounded-on-a-240-hz-one)
- [0156 — after a give-up, a fresh loss inside the settle window rewrites no surface, so the capture verdict says `live` while nothing is delivering](#0156--after-a-give-up-a-fresh-loss-inside-the-settle-window-rewrites-no-surface-so-the-capture-verdict-says-live-while-nothing-is-delivering)
- [0159 - an unrecognized flag is silently ignored and there is no `--help`, so a typo on a show night is indistinguishable from a network fault](#0159---an-unrecognized-flag-is-silently-ignored-and-there-is-no---help-so-a-typo-on-a-show-night-is-indistinguishable-from-a-network-fault)
- [0167 - six flags do nothing without `--stream`, and the roster that was built to end silently-ignored flags does not say so](#0167---six-flags-do-nothing-without---stream-and-the-roster-that-was-built-to-end-silently-ignored-flags-does-not-say-so)
- [0168 - the broken-literal defect is a class, and the guard Plan 0124 shipped is a six-item list](#0168---the-broken-literal-defect-is-a-class-and-the-guard-plan-0124-shipped-is-a-six-item-list)
- [0169 - `cargo doc` emits 64 intra-doc-link warnings and nothing in the project runs `cargo doc`](#0169---cargo-doc-emits-64-intra-doc-link-warnings-and-nothing-in-the-project-runs-cargo-doc)
- [0111 — `shot --render` spawns the encoder and builds a GPU device before it validates `--preset`](#0111--shot---render-spawns-the-encoder-and-builds-a-gpu-device-before-it-validates---preset)
- [0112 — the one canonical `ffmpeg` invocation is archival-grade and has no size lever](#0112--the-one-canonical-ffmpeg-invocation-is-archival-grade-and-has-no-size-lever)
- [0105 — the component's READ-ME-FIRST states the SDK it was built against, and on the pre-staged route nothing checks that claim](#0105--the-components-read-me-first-states-the-sdk-it-was-built-against-and-on-the-pre-staged-route-nothing-checks-that-claim)
- [0117 — the plugin's preset menu dispatches a snapshot index across a modal wait, and the "nothing can reload" argument is not sound](#0117--the-plugins-preset-menu-dispatches-a-snapshot-index-across-a-modal-wait-and-the-nothing-can-reload-argument-is-not-sound)
- [0118 — `foo_lmv.dll` has grown ~400 KB since the C ABI spec measured it, and the spec still advertises the old headroom](#0118--foo_lmvdll-has-grown-400-kb-since-the-c-abi-spec-measured-it-and-the-spec-still-advertises-the-old-headroom)
- [0130 — `boundary_density` scales with the capture resolution, and neither it nor its two floors names the 96x96 they were measured at](#0130--boundary_density-scales-with-the-capture-resolution-and-neither-it-nor-its-two-floors-names-the-96x96-they-were-measured-at)
- [0132 — the metrics module has no level statistic, and every statistic it does have reads gamma-encoded code values](#0132--the-metrics-module-has-no-level-statistic-and-every-statistic-it-does-have-reads-gamma-encoded-code-values)
- [0151 — the driven floor's sharpest non-vacuity probe is printed and never asserted, and it is the only one that separates "measures the music" from "measures nothing"](#0151--the-driven-floors-sharpest-non-vacuity-probe-is-printed-and-never-asserted-and-it-is-the-only-one-that-separates-measures-the-music-from-measures-nothing)
- [0152 — a disjunctive gate made "the shipped library's minimum" ambiguous, and `ANIM_FLOOR`'s recorded derivation still reads as though there were one population](#0152--a-disjunctive-gate-made-the-shipped-librarys-minimum-ambiguous-and-anim_floors-recorded-derivation-still-reads-as-though-there-were-one-population)
- [0135 — `parametric_curve` commits ~6.5 MB at Rich for buffers every shipped preset leaves empty](#0135--parametric_curve-commits-65-mb-at-rich-for-buffers-every-shipped-preset-leaves-empty)
- [0136 — a negative ring `scale` makes a `scallop` lobe bulge outward instead of dimpling inward](#0136--a-negative-ring-scale-makes-a-scallop-lobe-bulge-outward-instead-of-dimpling-inward)
- [0144 — the repaired `star` interior is exact only when the spikes are equal, and three places state it unconditionally](#0144--the-repaired-star-interior-is-exact-only-when-the-spikes-are-equal-and-three-places-state-it-unconditionally)
- [0104 — `check-index-rows.mjs` has no assertion that it can convict, so a dead detector reads exactly like a clean tree](#0104--check-index-rowsmjs-has-no-assertion-that-it-can-convict-so-a-dead-detector-reads-exactly-like-a-clean-tree)
- [0127 — the figure gate walks the working tree, so a gitignored local note can redden a push](#0127--the-figure-gate-walks-the-working-tree-so-a-gitignored-local-note-can-redden-a-push)
- [0133 — `docs-shots.mjs` cannot run at all, so the operator-doc image sweep has been dead since 2026-08-15](#0133--docs-shotsmjs-cannot-run-at-all-so-the-operator-doc-image-sweep-has-been-dead-since-2026-08-15)
- [0143 — 20 of the 24 `design-backlog.md#NNNN` anchors in the repo point at bodies that now live in the archive](#0143--20-of-the-24-design-backlogmdnnnn-anchors-in-the-repo-point-at-bodies-that-now-live-in-the-archive)
- [0162 - the claim gate resolves a probe path against the working tree, so a path inside `.gitignore` verifies locally and can only ever fail on CI](#0162---the-claim-gate-resolves-a-probe-path-against-the-working-tree-so-a-path-inside-gitignore-verifies-locally-and-can-only-ever-fail-on-ci)
- [0166 - the index-row gate measures a row's bytes and never its shape, so a closed-plan bullet dropped into the active-plans table passes](#0166---the-index-row-gate-measures-a-rows-bytes-and-never-its-shape-so-a-closed-plan-bullet-dropped-into-the-active-plans-table-passes)
- [0170 - the comment-hygiene gate walks the filesystem, so a gitignored vendored tree is invisible to CI and blocks every local push](#0170---the-comment-hygiene-gate-walks-the-filesystem-so-a-gitignored-vendored-tree-is-invisible-to-ci-and-blocks-every-local-push)
- [0171 - a backlog probe about a run of spaces is collapsed to one space before it is matched, so it can never fire](#0171---a-backlog-probe-about-a-run-of-spaces-is-collapsed-to-one-space-before-it-is-matched-so-it-can-never-fire)
- [0173 - the literal gate is blind to the defect in its unrejoined form, and the fixture README states that silence as a general truth](#0173---the-literal-gate-is-blind-to-the-defect-in-its-unrejoined-form-and-the-fixture-readme-states-that-silence-as-a-general-truth)
- [0134 — a joined corner is blunt, and the stroke that hid it is gone](#0134--a-joined-corner-is-blunt-and-the-stroke-that-hid-it-is-gone)
- [0174 — two of the four colour tags the render path pins do not survive into the container, and the doc calls all four the half most likely to ship wrong](#0174--two-of-the-four-colour-tags-the-render-path-pins-do-not-survive-into-the-container-and-the-doc-calls-all-four-the-half-most-likely-to-ship-wrong)
- [0175 — the render path's spend-nothing ordering is a structural property with no end-to-end guard](#0175--the-render-paths-spend-nothing-ordering-is-a-structural-property-with-no-end-to-end-guard)
- [0176 — `shot`'s usage text and its parser can drift with nothing checking, and the other CLI in this repo has exactly that guard](#0176--shots-usage-text-and-its-parser-can-drift-with-nothing-checking-and-the-other-cli-in-this-repo-has-exactly-that-guard)
- [0177 — the component's size cap has a trigger and no carrier: the recipe builds the DLL and never reads its own output's length](#0177--the-components-size-cap-has-a-trigger-and-no-carrier-the-recipe-builds-the-dll-and-never-reads-its-own-outputs-length)
- [0178 — the +510,464 B the component gained after 2026-08-18 is unattributed, and it is the larger half of the growth](#0178--the-510464-b-the-component-gained-after-2026-08-18-is-unattributed-and-it-is-the-larger-half-of-the-growth)
...
- **CLOSED 2026-08-27**, at [Plan 0121](plans/done/0121-a-rate-an-ink-edge-and-a-motion-reading.md)
  Phase 2. The column did not widen: a name longer than fourteen characters is now elided in the
  **middle** (`Tiled Rosette Mono` prints `Tiled R~e Mono`), because the distinguishing part of a
  name in this library is its tail. Three tests carry it — the known pair constructed explicitly
  rather than leaned on the shipped set, the fitter's head/tail behaviour, and a sweep asserting
  every shipped name fits to a label no other name fits to. Its own probe went red on delivery: it
  pinned the `{:<14.14}` truncation that was the defect.
...
- **CLOSED 2026-08-27**, at [Plan 0121](plans/done/0121-a-rate-an-ink-edge-and-a-motion-reading.md)
  Phase 3, on [ADR-0132](adrs/0132-a-rate-parameter-integrates-a-phase.md). `fragment_field` gained
  `field_speed` and `fold_speed`, both defaulting to `1.0` and both **integrating a phase** rather
  than scaling the shared clock, so a rate bound to audio bends the motion instead of teleporting
  it. The fold's designed 0.7 : 0.6 quadrature ratio stays welded inside `fold_speed`. Shipped
  content exercises both: `fragment_driftmono` binds bass and onset to `fold_speed` and bass to
  `field_speed`, and bought its intricacy back — `warp` 0.42 -> 0.62 with `fold_speed` 0.45, moving
  `cover` 0.450 -> 0.659 while `rate` fell 0.0411 -> 0.0370. Its own probe went red on delivery.
  **The rule the ADR states is engine-wide and its enumeration was not** — see 0141.
...
- **CLOSED 2026-08-27**, at [Plan 0121](plans/done/0121-a-rate-an-ink-edge-and-a-motion-reading.md)
  Phase 5, on [ADR-0133](adrs/0133-the-band-contour-fires-where-the-ink-changes.md). `band_contour`
  now samples the two band centres either side of the nearest edge, from both LUTs and crossfaded by
  `palette_mix` exactly as the main sample is, and suppresses the line when they agree to within
  half an 8-bit code value. Equality is the test, which is why no new parameter was needed and why
  no smooth palette moved at any step count. `shape_contourmono` turned the parameter on at `1.0`.
  **The entry says the function is written three times; it is written four** — the fourth copy, in
  `warp_mesh/mod.rs`, had already drifted (`dd` for `d`), which is precisely why the drift assertion
  never caught it: that test iterated three sites and the one place drift had happened was not
  among them. All four are canonical now and the assertion covers them. What the fix did **not**
  touch is *what* the contour draws — a soft scalar darken toward black, with no colour of its own
  — which is 0140.
...
- **CLOSED 2026-08-27**, at [Plan 0121](plans/done/0121-a-rate-an-ink-edge-and-a-motion-reading.md)
  Phase 1, on [ADR-0134](adrs/0134-motion-is-two-readings-and-anchoring-is-why-neither-can-be-a-threshold.md).
  `shot --report` gained `drive` (silence against full drive at the same depth) and `rate` (the mean
  consecutive-frame difference over the settled tail of the probe's loud plateau). Both ride captures
  the report already takes, so neither adds a render pass, a readback or a resize. **Both are printed
  readings and neither is a gate**, on this entry's own evidence: a raw rate does not rank
  watchability, because two comfortable presets measure higher than the draft rejected for shaking
  and the difference is anchoring, which no statistic here models. `rate` is measured at `PROBE_SIZE`
  (96x96) rather than `REPORT_SIZE` (192x192) and says so in the column note, in `docs/capturing.md`
...
> **CLOSED 2026-08-28** to [Plan 0127](plans/done/0127-the-picture-stops-depending-on-the-volume-slider.md)
> Phase 2, which dropped the `/aspect` at the point so `uv_to_world`'s multiply is the only aspect
> term. `a_straight_wave_trace_spans_one_over_aspect_of_the_width` was replaced by its inverse,
> asserted over 1:1, 4:3 and 16:9. **The entry's one unverified clause was then verified**: it said
> whether the reference spans the full width at every aspect "wants the same source" as 0120's
> constant, and Phase 3's reference capture read the x-extent at **1.000 of frame width at 16:9,
...
> **CLOSED 2026-08-28** to [ADR-0139](adrs/0139-the-waveform-is-levelled-at-the-analyzer-and-publishes-its-gain.md) +
> [Plan 0127](plans/done/0127-the-picture-stops-depending-on-the-volume-slider.md) Phase 1, which took the
> entry's own middle option: the trace is levelled at the analyzer against a slowly-released running
> peak of its magnitude, and the divisor is published as `AnalysisFrame::waveform_gain` so absolute
> amplitude stays one multiply away. Scale invariance is asserted as the property the entry argued
> for -- exact for a power-of-two gain change, and portable across a full 0.18x sweep through the
> real analyzer -- with a `WAVE_FLOOR` derived in amplitude units rather than copied from
> `BAND_FLOOR`. **Not re-measured in the live app**: the evidence is analyzer-level, and Phase 3
> read our side headlessly through `shot --audio`, where no volume control is in the path at all.
> The rebuilt component is installed on the development box if anyone wants the live confirmation.
...
- **CLOSED 2026-09-01** by [Plan 0149](plans/done/0149-the-line-corners-stop-being-blunt.md) Phase 4
  ...
  down to 5,760,008 B**, so **5,999,992 B** is not committed and a chord-web preset - every one in
  the shipped library - never reaches the path at all. `points` grew by one element for the
  off-by-one. `docs/nfr.md` section 12 was **false by roughly an order of magnitude** and was
  corrected rather than softened: it now bounds DSP and audio state, which is what it was measured
  against, with scene geometry named as a separate quantity. This entry probe went red on delivery.
...
- **CLOSED 2026-09-01** by [Plan 0149](plans/done/0149-the-line-corners-stop-being-blunt.md) Phase 3
  ...
  Every shipped preset still loads. **The refusal is a stated guess and remains one** - whether an
  inward scallop is a look anyone wants was not decided, and the refusal is what makes that ask come
  back as its own entry rather than shipping a silent bulge.
...
- **CLOSED 2026-09-01** by [Plan 0149](plans/done/0149-the-line-corners-stop-being-blunt.md) Phase 5
  ...
  Item 2 reads `shape_field`s `COORD_MODES`. Item 3 names the one divergence instead of promising
...
**CLOSED 2026-09-04** by [Plan 0138](plans/done/0138-the-colour-surface-stops-misleading-its-authors.md) Phase 4: a `shape_field` `color_span` whose figure interior falls below `MIN_INTERIOR_TEXELS = 16` now warns at load, naming the texel estimate and `palette_steps` as the remedy. The threshold is the middle of this entry’s own bracketing pair (8.6 read as upscaled, 32.3 did not), rounded to a power of two, and the property beside it is the one that holds: an interior drawn through N texels carries at most N colours however large it is on screen. Neither of the two non-fixes this entry names was taken. The whole shipped set loads warning-free, gated by an existing test over `EMBEDDED`; `shape_facet` at `color_span = 0.0521` is silent because it bands at 56 steps.
...
**CLOSED 2026-09-04** by [Plan 0138](plans/done/0138-the-colour-surface-stops-misleading-its-authors.md), both halves. The first — the standing question about the deferral — was answered by [ADR-0151](adrs/0151-palette-stops-are-authored-in-srgb-and-converted-at-load.md), which supersedes ADR-0021 Alternative E for palette stops: a stop is sRGB and is decoded to light once at the load boundary, and every shipped palette was re-based mechanically so the rendered output does not move. The second — the page — was rewritten in Phases 1 and 3. Both of this entry’s probes went red **on delivery**, on the two strings it was pinning. Verified at the close over every migrated stop pair in the diff (629 pairs, 1878 channels): 301 channels move one linear LUT level and all 301 encode to the identical display byte, so the migration is display-byte-exact.
...
- **CLOSED 2026-09-08** by [Plan 0140](plans/done/0140-every-rate-integrates-for-real.md). Its Phases 3 and 4 made the collage `drift`/`spin` and the emitter's sprite rotation integrate per element, and its Phase 1 corrected the `presets/README.md` row this entry's last probe convicts. **Four of the five `present:` probes were falsified by the repair itself** — they were written to go red on delivery rather than on decay, which is what `scripts/check-backlog-claims.mjs` reported at this close, and they retire with this body. The fifth (`collage_suprematist.toml`) still matches: no preset was retuned, deliberately, and Plan 0140's Phase 5 records why.
...
- **CLOSED 2026-09-08** by [Plan 0140](plans/done/0140-every-rate-integrates-for-real.md). Its Phase 2 sanitizes the frame delta once in `draw_frame` ([ADR-0152](adrs/0152-the-frame-delta-is-sanitized-at-the-scene-seam.md)) and deletes six in-scene copies — six, not the four this entry counted, because Plan 0126's splits had added two after it was written. The `swarm.rs` probe was falsified by the repair itself and retires with this body. **Two halves of the question outlived the plan and are live entries rather than part of this close:** 0189 (the seam sits one call below `self.time += dt`, so the engine's largest accumulator is still unguarded) and 0190 (the seam's comment names three downstream sites as unguarded and all three still carry a guard). `Phase::step` itself still takes `dt` on trust — that was the deliberate choice, not an omission: ADR-0152 puts the check at the seam rather than in the type.
...
[ADR-0181](adrs/0181-the-gate-compiles-every-feature-a-release-ships.md)'s `Outcome`.
...
push fired no run. That, and the fact that Phase 2 therefore removes a recovery path, are recorded
in [ADR-0181](adrs/0181-the-gate-compiles-every-feature-a-release-ships.md)'s `Outcome`. The wider
question of tags that produce no run at all is a new entry, 0196.
...
**CLOSED 2026-09-14** — [ADR-0191](adrs/0191-a-frame-delta-is-replaced-at-every-entry-and-nothing-below-keeps-a-policy.md) + [Plan 0171](plans/done/0171-one-stall-policy-and-a-guarded-clock.md) Phase 2. The policy landed on **step**: the
transition's guard is deleted, a degenerate frame reaches the dissolve as the entry's `FALLBACK_DT`
and advances it by that share, and `a_degenerate_dt_holds_progress` is replaced by
`a_degenerate_dt_advances_progress_by_the_nominal_share`. Both `present:` probes were falsified by
the repair, as written, and retire with this body. The price the entry named is the one ADR-0191's
Negative section accepts — a long stall no longer stretches a dissolve and may jump on resume — and
it is still unmeasured in either direction.
...
**CLOSED 2026-09-14** — [ADR-0191](adrs/0191-a-frame-delta-is-replaced-at-every-entry-and-nothing-below-keeps-a-policy.md) + [Plan 0171](plans/done/0171-one-stall-policy-and-a-guarded-clock.md) Phase 1. The guard went to neither place this entry
weighed alone: `sanitize_frame_dt` is called first by **every** `Renderer` entry that takes a
caller's delta — `render`, `render_tapped` and `capture_stream` — so the windowless path, which a
check hoisted into `render` would have missed, is covered too, and the C ABI stays untouched. **Both
`present:` probes still match after the repair**, and that is correct rather than a miss: they
reduced the claim to the two lines that carried the raw delta, and both lines still exist, now below
the substitution. What stands in their place is
`a_degenerate_frame_delta_steps_the_clock_by_the_nominal_step_at_every_entry`, which drives
`NaN`, `+inf`, `0.0` and `-1.0` through `render_tapped` and `capture_stream` and asserts the
clock advances by exactly `FALLBACK_DT`. `render` itself needs a presentable surface no test builds,
and was confirmed by reading at the close.
...
**CLOSED 2026-09-14** — [ADR-0191](adrs/0191-a-frame-delta-is-replaced-at-every-entry-and-nothing-below-keeps-a-policy.md) + [Plan 0171](plans/done/0171-one-stall-policy-and-a-guarded-clock.md) Phase 2. The decision went the other way from the
doc repair this entry sketched: the three guards are deleted and the comment is true again, now on
`sanitize_frame_dt`, and `core/tests/hygiene.rs` counts the finiteness-guard population so the
comment is gated rather than trusted. **The population was five, not three** — the scan found the
cellular `GenerationClock::advance` and the now-playing banner as well — and **three positivity-only
