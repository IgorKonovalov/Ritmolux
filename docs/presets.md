...
**A program wanting these names asks the player, not this page.** `ritmolux
--schema` prints a `grammar` object carrying three arrays — `variables`,
`functions` and `constants` — generated by walking the engine's own declarations,
so an expression editor colours exactly what the parser resolves and nothing
else. The prose below is the human reference: it says what each name *means*,
which is the half a roster cannot carry. See
