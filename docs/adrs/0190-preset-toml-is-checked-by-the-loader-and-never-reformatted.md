...
`nextest` run and needs no new CI wiring. For the editor, the engine's declarations are rendered as
JSON Schemas — **one self-contained file per system**, plus a generic file — committed with a
generated `.taplo.toml` that selects a library file's schema by its filename family (declared once per
system in the engine and read by the `file-name` rule as well), so Even Better TOML offers completion,
hover documentation and a diagnostic on an unknown key — **and nothing else from Taplo is adopted: no
formatter, no `taplo fmt --check`**.
...
- The JSON Schemas are a second rendering of the engine's declarations: sixteen committed generated
  files (fourteen per-system schemas of about 66 KB each, the generic one, and `.taplo.toml`), held
  to the engine by a drift test and regenerated by one env var. None of it ships.
- **Completion depends on the filename.** A file named off its family, every teaching file under
  `docs/examples/`, and the `[layer]` table get validation only. The unknown-key diagnostic lands on
  the `[params]` header rather than on the key, because the placement that reaches the key loses the
  key's name from the message.
- No gate can run the extension, so how Taplo treats globs and schemas is held by a recorded probe
  and a `human` phase, not by CI.
...
### Alternative D — one editor schema, keyed per system through `allOf` + `if`/`then`
This was the decision as first written. Rejected on Plan 0169 Phase 4's measurement against Even
Better TOML 0.21.2's bundled Taplo: validation honours `if`/`then`, but hover and completion ignore
it, so no parameter completed and none showed documentation. Keying the systems through `oneOf` or
`anyOf` instead merges hover text across systems, completes all 230 names, and reports a violation as
the whole document at line 1. One file cannot give completion scoped to the preset's system.

### Alternative E — per-system entry points into the one shared schema
Rejected on the same measurement: a rule whose schema path carries a `#/definitions/...` fragment
fails (the fragment is read as part of a filename), and a per-system file that only `$ref`s the
shared schema fails to resolve. Each per-system file has to be self-contained.
...
The 2026-09-12 editor measurements behind Alternatives D and E were taken the same way: a node script
driving the extension's own `dist/server.js` over IPC as VS Code launches it, reading
`taplo/associatedSchema`, hover, completion and published diagnostics. Plan 0169's Phase 4 outcome
carries the detail, including the Windows glob trap (an include glob must begin with `/`).
