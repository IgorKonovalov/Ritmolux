...
//! `core-cabi/include/rlx_core.h` separately from this crate, so any change to
...
//! - `rlx_push_samples` is called from at most one thread at a time (the
...
//!   host's UI/render thread), never concurrently with `rlx_create`/
//!   `rlx_free` on the same handle.
...
/// `rlx_load_presets` (ADR-0006); v3 added `rlx_set_debug` + `rlx_get_metrics`
/// and the `RlxMetrics` struct (ADR-0008); v4 added `rlx_render_dt` (ADR-0013);
/// v5 added `rlx_set_now_playing` (ADR-0110); v6 added `rlx_get_presets` +
/// `rlx_select_preset` (ADR-0117).
pub const RLX_ABI_VERSION: u32 = 6;

/// Call succeeded.
pub const RLX_OK: i32 = 0;
/// A null handle or otherwise invalid argument was passed.
pub const RLX_ERR_INVALID_ARG: i32 = -1;
/// The stream format was rejected at the boundary.
pub const RLX_ERR_FORMAT: i32 = -2;
/// Rendering failed (surface/device error).
pub const RLX_ERR_RENDER: i32 = -3;
/// A render call was made before a window was attached.
pub const RLX_ERR_NO_WINDOW: i32 = -4;
/// A Rust panic was caught at the boundary and mapped to an error.
pub const RLX_ERR_PANIC: i32 = -5;
/// The operation is unsupported on this platform.
pub const RLX_ERR_UNSUPPORTED: i32 = -6;

/// Debug flags (ADR-0008). No flags — a clean scene, metrics still collected.
pub const RLX_DEBUG_OFF: u32 = 0;
/// Draw the on-screen diagnostics overlay. Higher bits are reserved, ignored.
pub const RLX_DEBUG_OVERLAY: u32 = 1 << 0;

/// Environment variable read once at [`rlx_create`] to seed the default debug
...
/// [`rlx_set_debug`].
const DEBUG_OVERLAY_ENV: &str = "RLX_DEBUG_OVERLAY";
...
/// Read the `RLX_DEBUG_OVERLAY` env var once and map a truthy value to the
...
struct RenderState {
    ...
    scratch: Vec<f32>,
    /// Presets from `rlx_load_presets` called before a window was attached;
    /// installed on the renderer as soon as `rlx_attach_window` creates it.
    ...
    /// `rlx_set_debug`. Held here so a set-before-attach persists, then applied
    /// to the renderer when `rlx_attach_window` creates it.
    debug_flags: u32,
}
...
pub struct RlxHandle {
    channels: u16,
    producer: UnsafeCell<SampleProducer>,
    render: UnsafeCell<RenderState>,
}

/// The ABI version this build implements (see [`RLX_ABI_VERSION`]).
#[unsafe(no_mangle)]
pub extern "C" fn rlx_abi_version() -> u32 {
    RLX_ABI_VERSION
}
...
#[unsafe(no_mangle)]
pub extern "C" fn rlx_create(sample_rate: u32, channels: u16) -> *mut RlxHandle {
    catch_unwind(|| {
        ...
        Box::into_raw(Box::new(RlxHandle {
            channels,
...
///
/// # Safety
/// `handle` must be a pointer returned by `rlx_create`, not yet freed.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_free(handle: *mut RlxHandle) {
    if handle.is_null() {
        return;
    }
...
///
/// # Safety
/// `handle` valid per `rlx_create`; `samples` points at `sample_count` f32s.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_push_samples(
    handle: *mut RlxHandle,
    samples: *const f32,
    sample_count: u32,
) -> i32 {
...
///
/// # Safety
/// `handle` valid per `rlx_create`; `hwnd` a valid window handle outliving
/// this visualizer.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_attach_window(
    handle: *mut RlxHandle,
        ...
        catch_unwind(AssertUnwindSafe(|| {
            match unsafe { Renderer::new_from_surface_target(target, width, height) } {
                Ok(mut renderer) => {
                    ...
                    renderer.set_overlay(state.debug_flags & RLX_DEBUG_OVERLAY != 0);
                    state.renderer = Some(renderer);
                    RLX_OK
                }
                Err(_) => RLX_ERR_RENDER,
            }
        }))
        .unwrap_or(RLX_ERR_PANIC)
    }
...
///
/// # Safety
/// `handle` valid per `rlx_create`; render-thread role only.
#[unsafe(no_mangle)]
#[allow(
    clippy::indexing_slicing,
    reason = "n = pop_samples(&mut scratch) <= scratch.len(), so scratch[..n] is in range"
)]
pub unsafe extern "C" fn rlx_render_dt(handle: *mut RlxHandle, dt_seconds: f32) -> i32 {
    if handle.is_null() {
        return RLX_ERR_INVALID_ARG;
    }
    let handle = unsafe { &*handle };
...
/// Drain pending samples through analysis and draw one frame into the attached
/// window. The exact `1/60 s` wrapper over [`rlx_render_dt`] (ADR-0013) — a host
/// that has no real elapsed time keeps the original fixed-step behavior.
///
/// # Safety
/// `handle` valid per `rlx_create`; render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_render(handle: *mut RlxHandle) -> i32 {
    unsafe { rlx_render_dt(handle, 1.0 / 60.0) }
}
...
/// `handle` valid per `rlx_create`; render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_resize(handle: *mut RlxHandle, width: u32, height: u32) -> i32 {
    if handle.is_null() {
        return RLX_ERR_INVALID_ARG;
    }
    let handle = unsafe { &*handle };
...
/// inside the render loop off `rlx_render_dt`.
///
/// # Safety
/// `handle` valid per `rlx_create`; render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_cycle_scene(handle: *mut RlxHandle) -> i32 {
    if handle.is_null() {
        return RLX_ERR_INVALID_ARG;
    }
    let handle = unsafe { &*handle };
...
/// (`>= 0`), or a negative `RLX_ERR_*` code (null handle/path, invalid UTF-8).
...
/// no window is attached yet, the loaded set is applied when `rlx_attach_window`
/// next creates the renderer.
///
/// # Safety
/// `handle` valid per `rlx_create`; `path_utf8` points at `path_len` bytes of
/// UTF-8 text. Render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_load_presets(
    handle: *mut RlxHandle,
    path_utf8: *const u8,
    path_len: usize,
) -> i32 {
...
/// the total byte count the full list needs (`>= 0`), or a negative `RLX_ERR_*`.
///
...
/// flight, matching `rlx_cycle_scene`'s convention — or `-1` on an empty roster.
/// It is filled on every successful call, sizing calls included.
///
/// Returns `RLX_ERR_NO_WINDOW` before `rlx_attach_window`: the roster installs
...
/// same handle's roster with no `rlx_load_presets` in between. Added in ABI v6
/// (ADR-0117).
///
/// # Safety
/// `handle` valid per `rlx_create`; `buf` either null or writable for `buf_len`
/// bytes; `out_current_index` either null or a writable `int32_t`. Render-thread
/// role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_get_presets(
    handle: *mut RlxHandle,
    buf: *mut u8,
    buf_len: usize,
...
/// handle's [`rlx_get_presets`] reported — **dissolving** rather than cutting,
/// exactly as [`rlx_cycle_scene`] does.
///
/// Returns `RLX_OK`; `RLX_ERR_INVALID_ARG` for a null handle or an index that is
...
/// `RLX_ERR_NO_WINDOW` before `rlx_attach_window`. Added in ABI v6 (ADR-0117).
///
/// # Safety
/// `handle` valid per `rlx_create`; render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_select_preset(handle: *mut RlxHandle, index: i32) -> i32 {
    if handle.is_null() {
        return RLX_ERR_INVALID_ARG;
    }
    let handle = unsafe { &*handle };
...
///
/// Returns `RLX_OK`, or a negative `RLX_ERR_*`: `RLX_ERR_INVALID_ARG` for a null
...
/// and `RLX_ERR_NO_WINDOW` before a window is attached.
///
...
///
/// # Safety
/// `handle` valid per `rlx_create`; `utf8` points at `len` readable bytes.
/// Render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_set_now_playing(
    handle: *mut RlxHandle,
    utf8: *const u8,
    len: usize,
) -> i32 {
...
/// Set the debug flag set on the handle (`RLX_DEBUG_*`; higher bits reserved and
...
/// `handle` valid per `rlx_create`; render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_set_debug(handle: *mut RlxHandle, flags: u32) -> i32 {
    if handle.is_null() {
        return RLX_ERR_INVALID_ARG;
    }
    let handle = unsafe { &*handle };
    catch_unwind(AssertUnwindSafe(|| {
        ...
        if let Some(renderer) = state.renderer.as_mut() {
            renderer.set_overlay(flags & RLX_DEBUG_OVERLAY != 0);
        }
        RLX_OK
    }))
    .unwrap_or(RLX_ERR_PANIC)
...
/// caller sets `out->struct_size = sizeof(RlxMetrics)` first; the core writes at
...
/// allocation crosses the boundary. Returns `RLX_OK`, or `RLX_ERR_INVALID_ARG`
/// on a null handle/out. Added in ABI v3 (ADR-0008).
///
/// # Safety
/// `handle` valid per `rlx_create`; `out` points at a caller-allocated
/// `RlxMetrics` whose `struct_size` field is initialized. Render-thread role only.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rlx_get_metrics(handle: *mut RlxHandle, out: *mut RlxMetrics) -> i32 {
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let full = std::mem::size_of::<RlxMetrics>() as u32;
        ...
        let local = RlxMetrics {
            struct_size: n,
            abi_version: RLX_ABI_VERSION,
            fps: m.fps,
