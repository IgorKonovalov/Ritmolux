...
/// a `const MODE: u32` naming the pass (0 step, 1 seed, 2 stamp) and
/// [`gpu::HASH_WGSL`](crate::render::gpu::HASH_WGSL) are prepended at
/// construction, which is where `VsOut`, `MODE` and `mix32` come from. The
/// render target is the grid itself, so a fragment's position is its cell.
///
/// Everything here is integer arithmetic over texel values that are exactly 0
/// or 1, so a generation is the same on every adapter.
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    return vec4<f32>(state, 0.0, 0.0, 1.0);
}
"#;
...
// A live cell lights at the palette's origin; a dead one emits nothing.
fn paint(texel: vec4<f32>) -> Paint {
    var p: Paint;
    p.coord = 0.0;
    p.light = select(0.0, 1.0, texel.x > 0.5);
    return p;
}
