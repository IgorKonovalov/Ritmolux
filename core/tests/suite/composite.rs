...
//! tolerance. It is a **separate file** rather than two more arms of `golden.rs`,
//! following `background_composite.rs`'s posture: `golden` is a binary of its own
//! and nextest runs every test in a process of its own, so the feedback/fold
//! pipelines these fixtures build never coexist on a device with the per-system
//! golden captures. Building GPU resources mid-run
...
///
/// `RLX_BLESS=1 cargo test -p rlx-core --test suite composite::` rewrites these
/// two — and, run without the `composite::` filter or against the whole suite,
/// **other baselines as well**. Bless by that filter and check `git status`.
#[test]
fn composite_stages_match_golden_baselines() {
    ...
    for (stem, toml) in FIXTURES {
        ...
        assert!(
            path.exists(),
            "missing baseline {} — run `RLX_BLESS=1 cargo test -p rlx-core --test suite composite::`",
            path.display()
        );
