...
//!
//! They share **one** capture. A second `Renderer::new_headless` in this file
...
//! a second `spectrum` entry would break the invariant ADR-0023 rests on. Being
//! outside that binary also means
//! `RLX_BLESS=1 cargo test -p rlx-core --test suite line_joints::` rewrites
//! **this** baseline and cannot reach the roster.
...
///
/// `RLX_BLESS=1 cargo test -p rlx-core --test suite line_joints::` rewrites this
/// one baseline. Scoped to this module it **cannot** reach the `golden.rs` roster:
...
fn compare_against_baseline(img: &CaptureImage, renderer: &Renderer) {
    ...
    assert!(
        path.exists(),
        "missing baseline {} — run `RLX_BLESS=1 cargo test -p rlx-core --test suite line_joints::`",
        path.display()
    );
