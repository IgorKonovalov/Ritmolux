...
//! in while someone reads it as noise. As its own module,
//! `RLX_BLESS=1 cargo test -p rlx-core --test suite attractor_trails::` runs this
//! file's tests and reaches nothing else. **Drop the name filter and the bless
//! reaches every baseline test in the `suite` binary.** Same posture
...
//! fixture builds *six* pipelines. Rendering it inside another test would expose
//! that test's baselines to a device state they have never been rendered from;
//! nextest runs each test in a process of its own, and a bless filtered to this
//! module shares its process with nothing else.
//!
...
///
/// `RLX_BLESS=1 cargo test -p rlx-core --test suite attractor_trails::` rewrites
/// this baseline and — because the name filter selects this module — no other.
#[test]
fn the_attractor_over_the_trails_stage_matches_its_baseline() {
    ...
    assert!(
        path.exists(),
        "missing baseline {} — run `RLX_BLESS=1 cargo test -p rlx-core --test suite attractor_trails::`",
        path.display()
    );
...
/// The fixture still describes the configuration this file exists for.
///
