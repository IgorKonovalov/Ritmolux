...
//!     that override names nothing else (ADR-0193). No such exemption sits in a
//!     `tests/suite/` binary, where the override could not name it (ADR-0204).
    ...
    for file in &files {
        ...
        assert!(
            ...
            "hot-path module `{}` is missing the panic-denial pragma \
             ...
             from tests/suite/hygiene.rs to the top of the file.",
            file.display(),
        );
    }
}
...
/// Every `.rs` file under a workspace member's `tests/suite/`, as `(rel, text)`.
///
/// Those files are modules of one shared test binary, `suite`, so the run-alone
/// override's `binary()` terms cannot pick one of them out.
fn suite_test_files(root: &Path) -> Vec<(String, String)> {
    let mut files = Vec::new();
    for member in workspace_members(root) {
        let dir = root.join(&member).join("tests").join("suite");
        if !dir.is_dir() {
            continue;
        }
        for path in rs_files_under(&dir) {
            let rel = path
                .strip_prefix(root)
                .expect("a suite file is under the workspace root")
                .to_string_lossy()
                .replace('\\', "/");
            let text = std::fs::read_to_string(&path)
                .unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
...
/// The shared-binary files that carry a [`CLOCK_LINT`] exemption, each as a
/// failure line naming the file.
fn clock_readers_in_the_suite(files: &[(String, String)]) -> Vec<String> {
    files
        .iter()
        .filter(|(rel, text)| {
            clock_exemption(rel.clone(), String::new(), &strip_line_comments(text)).is_some()
        })
        .map(|(rel, _)| {
            format!(
                "{rel} carries a `{CLOCK_LINT}` exemption inside the shared `suite` binary, where \
                 the run-alone override cannot name it; a clock-reading test keeps a binary of its \
                 own (ADR-0204), so move it back to a top-level `tests/*.rs` and schedule it"
            )
        })
        .collect()
}
...
///
/// And no file under a member's `tests/suite/` carries the exemption at all: its
/// tests share the `suite` binary, which the filter cannot split (ADR-0204).
///
/// # What it cannot see
///
...
fn every_clock_reading_test_is_scheduled_alone() {
    ...
    let suite = suite_test_files(&root);
    assert!(
        suite
            .iter()
            .any(|(rel, _)| rel == "core/tests/suite/main.rs")
            && suite
                .iter()
                .any(|(rel, _)| rel == "standalone/tests/suite/main.rs"),
        "the suite scan no longer reaches both packages' `tests/suite/main.rs`, so it is \
         guarding less than it reads"
    );
    failures.extend(clock_readers_in_the_suite(&suite));
...
/// The suite check names a shared-binary file that exempts the clock lint, at
/// file or at function level, and passes one that only spells the lint's name.
#[test]
fn the_suite_clock_check_names_an_exemption_and_ignores_prose() {
    // `\x23` is `#`: spelled with an escape so this file, which the check scans,
    // carries no attribute of its own.
    let seeded = [
        (
            "core/tests/suite/file_level.rs".to_owned(),
            "\x23![allow(clippy::disallowed_methods, reason = \"reads Instant::now\")]\n\
             #[test]\nfn slope() {}\n"
                .to_owned(),
        ),
        (
            "standalone/tests/suite/fn_level.rs".to_owned(),
            "\x23[expect(clippy::disallowed_methods)]\n#[test]\nfn deadline() {}\n".to_owned(),
        ),
        (
            "core/tests/suite/prose.rs".to_owned(),
            "// \x23![allow(clippy::disallowed_methods)] is what a clock reader carries\n\
             const CLOCK_LINT: &str = \"clippy::disallowed_methods\";\n"
                .to_owned(),
        ),
    ];
    let failures = clock_readers_in_the_suite(&seeded);
    assert_eq!(failures.len(), 2, "{failures:#?}");
    assert!(failures[0].starts_with("core/tests/suite/file_level.rs carries"));
    assert!(failures[1].starts_with("standalone/tests/suite/fn_level.rs carries"));
}
