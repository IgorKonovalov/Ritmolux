...
/// One preset's reactivity gate, on a renderer the batch supplies. `Some` is
/// the conviction, ready to be printed with its siblings.
///
/// The claim is per preset and decomposes cleanly: a preset must answer at
/// least one band above [`REACTIVITY_FLOOR`], and no part of that is a
/// statement about the rest of the library — which is exactly what lets a
/// batch hold several without changing what any of them asserts.
fn reacts_to_at_least_one_band(renderer: &mut Renderer, name: &str) -> Option<String> {
    let system = default_presets()
        .into_iter()
        .find(|p| p.name == name)
        .map(|p| p.system)
        .unwrap_or_else(|| {
    ...
    let vector = measure(renderer, name);
    let max = max_of(&vector);
    println!(
        "[{}] {name:<10} bass={:.4} mid={:.4} treb={:.4} onset={:.4}  (max {:.4})",
        system_name(system),
        vector[0].1,
        vector[1].1,
        vector[2].1,
    ...
    (max < REACTIVITY_FLOOR)
        .then(|| format!("{name} reacts to no band above {REACTIVITY_FLOOR} (per-band {vector:?})"))
}
...
/// so a red run names all of them rather than stopping at the first — the batch
/// is a scheduling unit, and a reader who lost the one-test-per-preset naming
/// gets the whole verdict back in the message instead.
fn reacts_to_at_least_one_band_batch(names: &[&str]) {
    let Some(mut renderer) = common::headless(SIZE, SIZE) else {
        return;
    };
    let convicted: Vec<String> = names
        .iter()
        .filter_map(|name| reacts_to_at_least_one_band(&mut renderer, name))
        .collect();
    assert!(
        convicted.is_empty(),
        "{} of the {} presets in this batch react to no band:\n{}",
        convicted.len(),
        names.len(),
        convicted.join("\n")
    );
...
// (ADR-0157, ADR-0222) so a new `.toml` joins a batch by existing. nextest
// parallelizes across tests and never inside one, so the roster written as a
// single `#[test]` ran on one core.
include!(concat!(env!("OUT_DIR"), "/reactivity_tests.rs"));
