...
/// One preset's reactivity gate, as each generated `#[test]` calls it.
///
...
/// statement about the rest of the library.
fn reacts_to_at_least_one_band(name: &str) {
    ...
    assert!(
        ...
        "{name} reacts to no band above {REACTIVITY_FLOOR} (per-band {vector:?})"
    );
}

// One `#[test]` per shipped preset, generated by `core/build.rs` (ADR-0157) so
// a new `.toml` gets a test by existing. nextest parallelizes across tests and
// never inside one, so the roster written as a single `#[test]` ran on one core.
