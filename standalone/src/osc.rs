...
//! round-trip test in `tests.rs` is what holds the two to one padding rule.
//!
...
//! either way (ADR-0164). [`Telemetry::messages`] is the whole outbound table;
//! it is a fixed-size array so the roster cannot drift from what the sink sends.
//! The inbound vocabulary is [`decode::Action`], and
//! `docs/specs/0003-studio-control-protocol.md` is the authority on both.
//!
...
pub use encode::{ADDRESS_COUNT, ADDRESS_PREFIX, Arg, encode};
